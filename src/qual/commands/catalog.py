...
@dataclass(frozen=True)
class CommandHandlerActionRouteEntry:
    engine_action: str
    demo_path_step: str
    flow_step: str
    name: str
...
@dataclass(frozen=True)
class CommandHandlerActionRouteContract:
    entries: tuple[CommandHandlerActionRouteEntry, ...]
...
@dataclass(frozen=True)
class CommandDemoReadinessActionMatrixEntry:
    ordinal: int
    demo_path_step: str
    flow_step: str
    name: str
...
@dataclass(frozen=True)
class CommandDemoReadinessActionMatrixContract:
    entries: tuple[CommandDemoReadinessActionMatrixEntry, ...]
...
@dataclass(frozen=True)
class CommandDemoReadinessHandoffMapEntry:
    ordinal: int
    demo_path_step: str
    flow_step: str
    name: str
...
@dataclass(frozen=True)
class CommandDemoReadinessHandoffMapContract:
    entries: tuple[CommandDemoReadinessHandoffMapEntry, ...]
...
@dataclass(frozen=True)
class CommandDemoReadinessVerificationEntry:
    ordinal: int
    demo_path_step: str
    flow_step: str
    name: str
...
@dataclass(frozen=True)
class CommandDemoReadinessVerificationContract:
    is_complete: bool
    entries: tuple[CommandDemoReadinessVerificationEntry, ...]
...
class CommandDemoReadinessProgress:
    ...
    remaining_command_lines: tuple[str, ...]
...
_COMMAND_CLI_ENTRYPOINT_COMPATIBILITY_TOKENS: tuple[tuple[str, str], ...] = (
    ...
    ("retrieve-relevant-material", "context-basket"),
    ("search", "context-basket"),
    ("basket-add", "context-basket"),
    ("add-basket-item", "context-basket"),
    ("gather-context", "context-basket"),
    ("promote-context", "context-basket"),
    ("promote-retrieval-result", "context-basket"),
    ("revise", "diff-preview"),
...
_COMMAND_CLI_COMPATIBILITY_EXACT_ACTIONS: tuple[tuple[str, str], ...] = (
    ...
    ("retrieve-relevant-material", "ExegesisAppService.search_project"),
    ("search", "ExegesisAppService.search_project"),
    ("basket-add", "ExegesisAppService.add_basket_item"),
    ("add-basket-item", "ExegesisAppService.add_basket_item"),
    ("gather-context", "ExegesisAppService.add_basket_item"),
    ("promote-context", "ExegesisAppService.add_basket_item"),
    ("promote-retrieval-result", "ExegesisAppService.add_basket_item"),
...
def command_handler_action_route_contract(
    ...
    entries = tuple(
        CommandHandlerActionRouteEntry(
            ...
            name=entry.name,
            handler=handlers_by_name[entry.name].handler,
            delegated_to=handlers_by_name[entry.name].delegated_to,
    ...
    contract = CommandHandlerActionRouteContract(entries=entries)
    _validate_command_handler_action_route_contract(contract, specs=specs, launcher_argv=launcher_argv)
    return contract


def _validate_command_handler_action_route_contract(
    contract: CommandHandlerActionRouteContract,
    *,
    specs: tuple[CommandSpec, ...],
    launcher_argv: tuple[str, ...],
) -> None:
    exact_routes = command_demo_readiness_exact_action_route_contract(specs, launcher_argv).entries
    if tuple(entry.engine_action for entry in contract.entries) != tuple(
        entry.engine_action for entry in exact_routes
    ):
        raise ValueError("Command handler action route engine actions are inconsistent")
    handlers_by_name = {
        entry.name: entry
        for entry in command_handler_demo_path_contract(specs, launcher_argv).entries
    }
    for entry, exact_route in zip(contract.entries, exact_routes, strict=True):
        handler_entry = handlers_by_name.get(entry.name)
        if handler_entry is None:
            raise ValueError(f"Command handler action route is missing handler: {entry.name}")
        if (
            entry.demo_path_step != exact_route.demo_path_step
            or entry.flow_step != exact_route.flow_step
            or entry.name != exact_route.name
            or entry.command_argv != exact_route.command_argv
...
def command_handler_action_route_summary(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[tuple[str, str, str, str, str, str, tuple[str, ...], str], ...]:
    return tuple(
        (
            entry.engine_action,
            entry.demo_path_step,
...
def command_handler_action_route_lookup_table(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[tuple[str, tuple[str, str, tuple[str, ...]]], ...]:
    return tuple(
        (entry.engine_action, (entry.handler, entry.delegated_to, entry.command_argv))
        for entry in command_handler_action_route_contract(specs, launcher_argv).entries
    )


def command_handler_action_route_entry_for_engine_action(
    engine_action: str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
...
def command_mvp_handler_action_route_contract(
    ...
    return command_handler_action_route_contract(specs, launcher_argv)


def command_mvp_handler_action_route_summary(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[tuple[str, str, str, str, str, str, tuple[str, ...], str], ...]:
    return command_handler_action_route_summary(specs, launcher_argv)


def command_mvp_handler_action_route_lookup_table(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[tuple[str, tuple[str, str, tuple[str, ...]]], ...]:
    return command_handler_action_route_lookup_table(specs, launcher_argv)


def command_mvp_handler_action_route_entry_for_engine_action(
    engine_action: str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandHandlerActionRouteEntry | None:
...
@lru_cache(maxsize=None)
def command_demo_readiness_handoff_map_contract(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
    ...
    for index, step in enumerate(action_steps):
        ...
        entries.append(
            CommandDemoReadinessHandoffMapEntry(
                ordinal=step.ordinal,
                demo_path_step=step.demo_path_step,
                flow_step=step.flow_step,
                name=step.name,
                command_argv=_argv_without_launcher(
                    command_demo_readiness_argv_for_flow_step(step.flow_step, specs, launcher_argv),
                    launcher_argv,
                ),
                command_line=step.command_line,
    ...
    contract = CommandDemoReadinessHandoffMapContract(entries=tuple(entries))
    _validate_command_demo_readiness_handoff_map_contract(contract, action_steps, specs, launcher_argv)
    return contract


def _validate_command_demo_readiness_handoff_map_contract(
    contract: CommandDemoReadinessHandoffMapContract,
    action_steps: tuple[CommandDemoReadinessHandoffActionStep, ...],
    specs: tuple[CommandSpec, ...],
    launcher_argv: tuple[str, ...],
) -> None:
    if tuple(entry.ordinal for entry in contract.entries) != tuple(step.ordinal for step in action_steps):
        raise ValueError("Command demo readiness handoff map ordinals are inconsistent")
    if tuple(entry.demo_path_step for entry in contract.entries) != tuple(
        step.demo_path_step for step in action_steps
    ):
        raise ValueError("Command demo readiness handoff map path steps are inconsistent")
    if tuple(entry.flow_step for entry in contract.entries) != tuple(step.flow_step for step in action_steps):
        raise ValueError("Command demo readiness handoff map flow steps are inconsistent")
    if tuple(entry.name for entry in contract.entries) != tuple(step.name for step in action_steps):
        raise ValueError("Command demo readiness handoff map command names are inconsistent")
    if tuple(entry.command_line for entry in contract.entries) != tuple(step.command_line for step in action_steps):
        raise ValueError("Command demo readiness handoff map command lines are inconsistent")
    if tuple(entry.exact_action_lines for entry in contract.entries) != tuple(
    ...
    if tuple(entry.next_demo_path_step for entry in contract.entries) != (
        *(step.demo_path_step for step in action_steps[1:]),
        ...
        raise ValueError("Command demo readiness handoff map next steps are inconsistent")
    for entry in contract.entries:
        expected_argv = _argv_without_launcher(
            command_demo_readiness_argv_for_flow_step(entry.flow_step, specs, launcher_argv),
            launcher_argv,
        )
...
def command_demo_readiness_handoff_map_payload(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[dict[str, object], ...]:
    return tuple(
...
def command_demo_readiness_handoff_map_json(
    ...
    return json.dumps(
        command_demo_readiness_handoff_map_payload(specs, launcher_argv),
        sort_keys=True,
        separators=(",", ":"),
    )
...
def _validate_command_demo_readiness_handoff_packet(
    ...
    if not packet.tasks_completed:
        raise ValueError("Command demo readiness handoff packet tasks must not be empty")
    if tuple(f"{index}. " for index in range(1, len(packet.tasks_completed) + 1)) != tuple(
        task[: len(f"{index}. ")]
        for index, task in enumerate(packet.tasks_completed, start=1)
    ):
...
def _command_demo_readiness_handoff_packet_payload(
    ...
    return {
        ...
        "tasks_completed": list(packet.tasks_completed),
...
def _command_demo_readiness_handoff_field_entries(
    ...
    return (
        ...
        CommandDemoReadinessHandoffFieldEntry("tasks_completed", "; ".join(packet.tasks_completed)),
...
def _validate_command_demo_readiness_handoff_field_contract(
    ...
    if values_by_name["tasks_completed"] != "; ".join(packet.tasks_completed):
        raise ValueError("Command demo readiness handoff tasks field is inconsistent")
...
def command_demo_readiness_handoff_packet_markdown(
    ...
    lines = (
        ...
        f"- Tasks completed: {'; '.join(packet.tasks_completed)}",
...
def _validate_command_demo_readiness_handoff_packet_markdown(
    ...
    if "Tasks completed" not in markdown:
        raise ValueError("Command demo readiness handoff packet markdown lacks task fields")
...
@lru_cache(maxsize=None)
def command_demo_readiness_verification_contract(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
    ...
    entries = tuple(
        CommandDemoReadinessVerificationEntry(
            ordinal=step.ordinal,
            demo_path_step=step.demo_path_step,
            flow_step=step.flow_step,
            name=step.name,
            command_line=step.command_line,
            exact_action_lines=step.exact_action_lines,
            status="ready" if step.is_complete else "blocked",
        )
        for step in status_contract.steps
    )
    contract = CommandDemoReadinessVerificationContract(
        is_complete=status_contract.is_complete and all(entry.status == "ready" for entry in entries),
        entries=entries,
        missing_steps=tuple(entry.flow_step for entry in entries if entry.status != "ready"),
    )
    _validate_command_demo_readiness_verification_contract(contract, status_contract)
    return contract


def _validate_command_demo_readiness_verification_contract(
    contract: CommandDemoReadinessVerificationContract,
    status_contract: CommandDemoReadinessHandoffStepStatusContract,
) -> None:
    if tuple(entry.ordinal for entry in contract.entries) != tuple(step.ordinal for step in status_contract.steps):
        raise ValueError("Command demo readiness verification ordinals are inconsistent")
    if tuple(entry.demo_path_step for entry in contract.entries) != tuple(
        step.demo_path_step for step in status_contract.steps
    ):
        raise ValueError("Command demo readiness verification demo path steps are inconsistent")
    if tuple(entry.flow_step for entry in contract.entries) != tuple(step.flow_step for step in status_contract.steps):
        raise ValueError("Command demo readiness verification flow steps are inconsistent")
    if tuple(entry.name for entry in contract.entries) != tuple(step.name for step in status_contract.steps):
        raise ValueError("Command demo readiness verification commands are inconsistent")
    if tuple(entry.command_line for entry in contract.entries) != tuple(
        step.command_line for step in status_contract.steps
    ):
        raise ValueError("Command demo readiness verification command lines are inconsistent")
    if tuple(entry.exact_action_lines for entry in contract.entries) != tuple(
        step.exact_action_lines for step in status_contract.steps
    ):
        raise ValueError("Command demo readiness verification exact action lines are inconsistent")
    expected_statuses = tuple("ready" if step.is_complete else "blocked" for step in status_contract.steps)
    if tuple(entry.status for entry in contract.entries) != expected_statuses:
        raise ValueError("Command demo readiness verification statuses are inconsistent")
    expected_missing_steps = tuple(
        step.flow_step
        for step in status_contract.steps
        if not step.is_complete
    )
    if contract.missing_steps != expected_missing_steps:
        raise ValueError("Command demo readiness verification missing steps are inconsistent")
    if contract.is_complete != (status_contract.is_complete and not contract.missing_steps):
        raise ValueError("Command demo readiness verification completeness is inconsistent")


def command_demo_readiness_verification_summary(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[bool, tuple[str, ...], tuple[tuple[int, str, str, str, str, tuple[tuple[str, str], ...], str], ...]]:
    contract = command_demo_readiness_verification_contract(specs, launcher_argv)
    return (
        contract.is_complete,
        contract.missing_steps,
        tuple(
            (
                entry.ordinal,
                entry.demo_path_step,
...
def command_demo_smoke_sequence_issues(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[str, ...]:
    contract = command_demo_smoke_sequence_contract(specs, launcher_argv)
    ...
    for entry in contract.entries:
        if not entry.ready:
            issues.append(f"step not ready: {entry.name}")
        if not entry.thin_handler_ready:
            issues.append(f"handler not thin: {entry.name}")
        if not entry.smoke_argv:
            issues.append(f"missing smoke argv: {entry.name}")
        if not entry.exact_action_argv:
            issues.append(f"missing exact action argv: {entry.name}")
    return tuple(issues)


def require_command_demo_smoke_sequence_complete(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandDemoSmokeSequenceContract:
    contract = command_demo_smoke_sequence_contract(specs, launcher_argv)
...
def command_demo_readiness_verification_payload(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> dict[str, object]:
    contract = command_demo_readiness_verification_contract(specs, launcher_argv)
    payload: dict[str, object] = {
        "is_complete": contract.is_complete,
        "missing_steps": list(contract.missing_steps),
        "entries": [
            {
                "ordinal": entry.ordinal,
                "demo_path_step": entry.demo_path_step,
...
def command_demo_readiness_verification_json(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> str:
    return json.dumps(
        command_demo_readiness_verification_payload(specs, launcher_argv),
        sort_keys=True,
        separators=(",", ":"),
    )
...
def command_demo_readiness_exact_action_route_index(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[tuple[str, CommandDemoReadinessExactActionRouteEntry], ...]:
    return tuple(
        (entry.engine_action, entry)
        for entry in command_demo_readiness_exact_action_route_contract(
            specs,
...
def command_demo_readiness_exact_action_route_for_engine_action(
    engine_action: str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandDemoReadinessExactActionRouteEntry | None:
    requested_action = engine_action.strip()
    if not requested_action:
        return None
    return dict(command_demo_readiness_exact_action_route_index(specs, launcher_argv)).get(
        requested_action
    )
...
def command_demo_readiness_cli_exact_action_entry_for_argv(
    argv: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandDemoReadinessExactActionEntry | None:
    engine_action = command_demo_readiness_cli_exact_action_for_argv(argv, specs, launcher_argv)
    if engine_action is None:
        return None
...
@lru_cache(maxsize=None)
def command_demo_readiness_action_matrix_contract(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandDemoReadinessActionMatrixContract:
    route_contract = command_demo_readiness_route_contract(specs, launcher_argv)
    exact_entries_by_flow_step = {
        flow_step: entries
        for flow_step, entries in command_demo_readiness_exact_action_index_by_flow_step(
            specs,
            launcher_argv,
        )
    }
    contract = CommandDemoReadinessActionMatrixContract(
        entries=tuple(
            CommandDemoReadinessActionMatrixEntry(
                ordinal=entry.ordinal,
                demo_path_step=entry.demo_path_step,
                flow_step=entry.flow_step,
                name=entry.name,
                engine_actions=entry.engine_actions,
                exact_action_lines=tuple(
                    (exact_entry.engine_action, exact_entry.command_line)
                    for exact_entry in exact_entries_by_flow_step.get(entry.flow_step, ())
                ),
            )
            for entry in route_contract.entries
        )
    )
    _validate_command_demo_readiness_action_matrix_contract(
        contract,
        route_contract,
        specs,
        launcher_argv,
...
def _validate_command_demo_readiness_action_matrix_contract(
    contract: CommandDemoReadinessActionMatrixContract,
    route_contract: CommandDemoReadinessRouteContract,
    specs: tuple[CommandSpec, ...],
    launcher_argv: tuple[str, ...],
) -> None:
    route_entries = route_contract.entries
    if tuple(entry.ordinal for entry in contract.entries) != tuple(entry.ordinal for entry in route_entries):
        raise ValueError("Command demo readiness action matrix ordinals are inconsistent")
    if tuple(entry.demo_path_step for entry in contract.entries) != tuple(
        entry.demo_path_step for entry in route_entries
    ):
        raise ValueError("Command demo readiness action matrix path steps are inconsistent")
    if tuple(entry.flow_step for entry in contract.entries) != tuple(entry.flow_step for entry in route_entries):
        raise ValueError("Command demo readiness action matrix flow steps are inconsistent")
    if tuple(entry.name for entry in contract.entries) != tuple(entry.name for entry in route_entries):
        raise ValueError("Command demo readiness action matrix command names are inconsistent")
    if tuple(entry.engine_actions for entry in contract.entries) != tuple(
        entry.engine_actions for entry in route_entries
    ):
        raise ValueError("Command demo readiness action matrix actions are inconsistent")

    flattened_action_lines = tuple(
        action_line
        for entry in contract.entries
        for action_line in entry.exact_action_lines
    )
    if flattened_action_lines != command_demo_readiness_exact_action_line_lookup_table(specs, launcher_argv):
        raise ValueError("Command demo readiness action matrix exact lines are inconsistent")

    for entry in contract.entries:
        if not entry.engine_actions or not entry.exact_action_lines:
            raise ValueError(f"Command demo readiness action matrix entry is incomplete: {entry.flow_step}")
        if tuple(engine_action for engine_action, _ in entry.exact_action_lines) != entry.engine_actions:
            raise ValueError(
                f"Command demo readiness action matrix coverage is inconsistent: {entry.flow_step}"
            )
        for engine_action, command_line in entry.exact_action_lines:
            resolved_action = command_demo_readiness_exact_action_for_argv(
                command_line,
            ...
            if resolved_action != engine_action:
                raise ValueError(
                    f"Command demo readiness action matrix line is not exact: {engine_action}"
                )


def command_demo_readiness_action_matrix_summary(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
    ...
    return tuple(
        ...
        for entry in command_demo_readiness_action_matrix_contract(specs, launcher_argv).entries
    )
...
def command_demo_readiness_primary_engine_action_for_argv(
    argv: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> str | None:
    ...
    if exact_action is not None:
        return exact_action
    action_matches = command_demo_readiness_engine_action_matches_for_argv(argv, specs, launcher_argv)
    if len(action_matches) == 1:
        return action_matches[0]
    return None


def command_demo_readiness_primary_action_line_for_argv(
    argv: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[str, str] | None:
    primary_action = command_demo_readiness_primary_engine_action_for_argv(argv, specs, launcher_argv)
    ...
    if exact_line:
        return primary_action, exact_line
    for engine_action, action_line in command_demo_readiness_action_lines_for_argv(
        argv,
        specs,
        launcher_argv,
    ):
        if engine_action == primary_action:
            return engine_action, action_line
    return None
...
def _remaining_command_lines_for_validation(
    validation: CommandDemoReadinessScriptValidation,
    specs: tuple[CommandSpec, ...],
    launcher_argv: tuple[str, ...],
) -> tuple[str, ...]:
...
def _remaining_exact_action_lines_for_validation(
    validation: CommandDemoReadinessScriptValidation,
    specs: tuple[CommandSpec, ...],
    launcher_argv: tuple[str, ...],
) -> tuple[str, ...]:
    return tuple(
        command_demo_readiness_exact_line_for_engine_action(engine_action, specs, launcher_argv)
        for engine_action in validation.missing_engine_actions
    )


def _next_flow_step_for_validation(
    validation: CommandDemoReadinessScriptValidation,
    specs: tuple[CommandSpec, ...],
    launcher_argv: tuple[str, ...],
) -> str | None:
    if validation.missing_engine_actions:
        return command_demo_action_flow_step(
            validation.missing_engine_actions[0],
            specs,
        )
    if validation.missing_flow_steps:
        return validation.missing_flow_steps[0]
    return None


def _validate_command_demo_readiness_progress(
    progress: CommandDemoReadinessProgress,
    specs: tuple[CommandSpec, ...],
    launcher_argv: tuple[str, ...],
) -> None:
    expected_command_lines = _remaining_command_lines_for_validation(
        progress.validation,
        specs,
        launcher_argv,
    )
    ...
    if progress.remaining_command_lines != expected_command_lines:
        raise ValueError("Command demo readiness progress command lines are inconsistent")
    if progress.remaining_exact_action_lines != expected_exact_action_lines:
        raise ValueError("Command demo readiness progress exact action lines are inconsistent")
    expected_next_flow_step = _next_flow_step_for_validation(
        progress.validation,
        specs,
        launcher_argv,
    )
    if progress.next_flow_step != expected_next_flow_step:
        raise ValueError("Command demo readiness progress next flow step is inconsistent")
    expected_next_command_line = expected_command_lines[0] if expected_command_lines else ""
    if progress.next_command_line != expected_next_command_line:
        raise ValueError("Command demo readiness progress next command line is inconsistent")
    expected_next_exact_action_line = expected_exact_action_lines[0] if expected_exact_action_lines else ""
    if progress.next_exact_action_line != expected_next_exact_action_line:
        raise ValueError("Command demo readiness progress next exact action line is inconsistent")
    if progress.validation.is_complete and (
        progress.next_flow_step is not None
        or progress.next_command_line
        or progress.next_exact_action_line
        or progress.remaining_command_lines
        or progress.remaining_exact_action_lines
    ):
        raise ValueError("Command demo readiness progress is complete but still has remaining work")


def command_demo_readiness_progress(
    argvs: Sequence[Sequence[str] | str],
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandDemoReadinessProgress:
    validation = command_demo_readiness_validate_cli_script(argvs, specs, launcher_argv)
...
def command_demo_readiness_progress_summary(
    argvs: Sequence[Sequence[str] | str],
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
...
def command_demo_readiness_command_progress_contract(
    ...
    progress = command_demo_readiness_progress(argvs, specs, launcher_argv)
    return _command_demo_readiness_command_progress_contract_for_progress(
        progress,
        specs,
        launcher_argv,
    )
...
def command_demo_readiness_handoff_command_progress_contract(
    argvs: Sequence[Sequence[str] | str],
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandDemoReadinessCommandProgressContract:
...
def _command_demo_readiness_command_progress_contract_for_progress(
    progress: CommandDemoReadinessProgress,
    specs: tuple[CommandSpec, ...],
    launcher_argv: tuple[str, ...],
) -> CommandDemoReadinessCommandProgressContract:
...
def command_demo_readiness_command_progress_summary(
    ...
    return _command_demo_readiness_command_progress_summary(contract)


def command_demo_readiness_handoff_command_progress_summary(
    argvs: Sequence[Sequence[str] | str],
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[
    ...
    contract = command_demo_readiness_handoff_command_progress_contract(
        argvs,
        specs,
        launcher_argv,
    )
...
def _command_demo_readiness_command_progress_summary(
    contract: CommandDemoReadinessCommandProgressContract,
) -> tuple[
    bool,
    str | None,
...
def command_demo_readiness_command_progress_payload(
    ...
    contract = command_demo_readiness_command_progress_contract(argvs, specs, launcher_argv)
    return _command_demo_readiness_command_progress_payload(contract)


def command_demo_readiness_handoff_command_progress_payload(
    argvs: Sequence[Sequence[str] | str],
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> dict[str, object]:
...
def _command_demo_readiness_command_progress_payload(
    contract: CommandDemoReadinessCommandProgressContract,
) -> dict[str, object]:
    return {
        "is_complete": contract.is_complete,
...
def command_demo_readiness_handoff_command_progress_json(
    argvs: Sequence[Sequence[str] | str],
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> str:
    return json.dumps(
        command_demo_readiness_handoff_command_progress_payload(argvs, specs, launcher_argv),
        sort_keys=True,
        separators=(",", ":"),
    )
...
def command_demo_readiness_shell_progress(
    ...
    return command_demo_readiness_progress(
...
def command_demo_readiness_shell_progress_summary(
    lines: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[bool, str | None, str, str, tuple[str, ...], tuple[str, ...], tuple[tuple[str, ...], ...]]:
    progress = command_demo_readiness_shell_progress(lines, specs, launcher_argv)
    return (
        progress.validation.is_complete,
        progress.next_flow_step,
        progress.next_command_line,
...
def command_demo_readiness_shell_handoff_command_progress_contract(
    lines: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandDemoReadinessCommandProgressContract:
    return command_demo_readiness_handoff_command_progress_contract(
        _shell_script_executable_argv(lines),
        specs,
        launcher_argv,
    )
...
def command_demo_readiness_shell_handoff_command_progress_summary(
    lines: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[
    ...
    return command_demo_readiness_handoff_command_progress_summary(
        _shell_script_executable_argv(lines),
        specs,
        launcher_argv,
    )
...
def command_demo_readiness_shell_handoff_command_progress_payload(
    lines: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> dict[str, object]:
    return command_demo_readiness_handoff_command_progress_payload(
        _shell_script_executable_argv(lines),
        specs,
        launcher_argv,
    )
...
def command_demo_readiness_shell_handoff_command_progress_json(
    lines: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> str:
    return command_demo_readiness_handoff_command_progress_json(
        _shell_script_executable_argv(lines),
        specs,
        launcher_argv,
    )
...
def _command_demo_readiness_next_action_payload(
    ...
    validation = next_action.validation
    return {
        "is_complete": next_action.is_complete,
        "next_engine_action": next_action.next_engine_action,
        "next_demo_path_step": next_action.next_demo_path_step,
        "next_flow_step": next_action.next_flow_step,
        "next_command_line": next_action.next_command_line,
        "next_exact_action_line": next_action.next_exact_action_line,
        "remaining_engine_actions": next_action.remaining_engine_actions,
        "remaining_exact_action_lines": next_action.remaining_exact_action_lines,
        "invalid_argv": next_action.invalid_argv,
        "validation": {
...
def command_demo_readiness_next_action_payload(
    argvs: Sequence[Sequence[str] | str],
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> dict[str, object]:
    return _command_demo_readiness_next_action_payload(
        command_demo_readiness_next_action(argvs, specs, launcher_argv)
    )
...
def command_demo_readiness_next_action_json(
    ...
    return json.dumps(
        command_demo_readiness_next_action_payload(argvs, specs, launcher_argv),
        sort_keys=True,
        separators=(",", ":"),
    )
...
def command_demo_readiness_next_command_argv(
    ...
    next_action = command_demo_readiness_next_action(argvs, specs, launcher_argv)
    if next_action.next_flow_step is None:
        return ()
    return command_demo_readiness_argv_for_flow_step(
        next_action.next_flow_step,
        specs,
        launcher_argv,
    )
...
def command_demo_readiness_next_exact_action_argv(
    argvs: Sequence[Sequence[str] | str],
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[str, ...]:
    next_action = command_demo_readiness_next_action(argvs, specs, launcher_argv)
    if next_action.next_engine_action is None:
        return ()
...
def command_demo_readiness_shell_next_action_payload(
    ...
    return command_demo_readiness_next_action_payload(
        _shell_script_executable_argv(lines),
        specs,
        launcher_argv,
    )
...
def command_demo_readiness_shell_next_command_argv(
    lines: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[str, ...]:
    return command_demo_readiness_next_command_argv(
        _shell_script_executable_argv(lines),
        specs,
        launcher_argv,
    )
...
def command_demo_readiness_shell_next_exact_action_argv(
    lines: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[str, ...]:
    return command_demo_readiness_next_exact_action_argv(
        _shell_script_executable_argv(lines),
        specs,
        launcher_argv,
    )
...
def command_mvp_demo_readiness_progress(
    argvs: Sequence[Sequence[str] | str],
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandDemoReadinessProgress:
    return command_demo_readiness_progress(argvs, specs, launcher_argv)


def command_mvp_demo_readiness_progress_summary(
    argvs: Sequence[Sequence[str] | str],
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[bool, str | None, str, str, tuple[str, ...], tuple[str, ...], tuple[tuple[str, ...], ...]]:
...
def command_mvp_demo_readiness_handoff_command_progress_contract(
    argvs: Sequence[Sequence[str] | str],
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandDemoReadinessCommandProgressContract:
    return command_demo_readiness_handoff_command_progress_contract(argvs, specs, launcher_argv)
...
def command_mvp_demo_readiness_handoff_command_progress_summary(
    argvs: Sequence[Sequence[str] | str],
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[
    ...
    return command_demo_readiness_handoff_command_progress_summary(argvs, specs, launcher_argv)
...
def command_mvp_demo_readiness_handoff_command_progress_payload(
    argvs: Sequence[Sequence[str] | str],
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> dict[str, object]:
    return command_demo_readiness_handoff_command_progress_payload(argvs, specs, launcher_argv)
...
def command_mvp_demo_readiness_handoff_command_progress_json(
    argvs: Sequence[Sequence[str] | str],
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> str:
    return command_demo_readiness_handoff_command_progress_json(argvs, specs, launcher_argv)
...
def command_mvp_demo_readiness_shell_progress(
    lines: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandDemoReadinessProgress:
    return command_demo_readiness_shell_progress(lines, specs, launcher_argv)


def command_mvp_demo_readiness_shell_progress_summary(
    lines: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[bool, str | None, str, str, tuple[str, ...], tuple[str, ...], tuple[tuple[str, ...], ...]]:
...
def command_mvp_demo_readiness_shell_handoff_command_progress_contract(
    lines: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandDemoReadinessCommandProgressContract:
    return command_demo_readiness_shell_handoff_command_progress_contract(lines, specs, launcher_argv)
...
def command_mvp_demo_readiness_shell_handoff_command_progress_summary(
    lines: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[
    ...
    return command_demo_readiness_shell_handoff_command_progress_summary(lines, specs, launcher_argv)
...
def command_mvp_demo_readiness_shell_handoff_command_progress_payload(
    lines: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> dict[str, object]:
    return command_demo_readiness_shell_handoff_command_progress_payload(lines, specs, launcher_argv)
...
def command_mvp_demo_readiness_shell_handoff_command_progress_json(
    lines: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> str:
    return command_demo_readiness_shell_handoff_command_progress_json(lines, specs, launcher_argv)
...
def command_mvp_demo_readiness_next_command_argv(
    ...
    return command_demo_readiness_next_command_argv(argvs, specs, launcher_argv)
...
def command_mvp_demo_readiness_shell_next_command_argv(
    ...
    return command_demo_readiness_shell_next_command_argv(lines, specs, launcher_argv)


def command_mvp_demo_readiness_shell_next_exact_action_argv(
    lines: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[str, ...]:
...
def command_mvp_demo_readiness_handoff_map_contract(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandDemoReadinessHandoffMapContract:
    return command_demo_readiness_handoff_map_contract(specs, launcher_argv)


def command_mvp_demo_readiness_handoff_map_summary(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[tuple[int, str, str, str, tuple[str, ...], str, tuple[tuple[str, str], ...], str | None], ...]:
    return command_demo_readiness_handoff_map_summary(specs, launcher_argv)


def command_mvp_demo_readiness_handoff_map_payload(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[dict[str, object], ...]:
    return command_demo_readiness_handoff_map_payload(specs, launcher_argv)


def command_mvp_demo_readiness_handoff_map_json(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> str:
...
def command_mvp_demo_readiness_verification_contract(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandDemoReadinessVerificationContract:
    return command_demo_readiness_verification_contract(specs, launcher_argv)


def command_mvp_demo_readiness_verification_summary(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[bool, tuple[str, ...], tuple[tuple[int, str, str, str, str, tuple[tuple[str, str], ...], str], ...]]:
    return command_demo_readiness_verification_summary(specs, launcher_argv)
...
def command_mvp_demo_smoke_sequence_issues(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[str, ...]:
    return command_demo_smoke_sequence_issues(specs, launcher_argv)


def require_command_mvp_demo_smoke_sequence_complete(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandDemoSmokeSequenceContract:
...
def command_mvp_demo_readiness_verification_payload(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> dict[str, object]:
    return command_demo_readiness_verification_payload(specs, launcher_argv)


def command_mvp_demo_readiness_verification_json(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> str:
    return command_demo_readiness_verification_json(specs, launcher_argv)
...
def command_mvp_demo_readiness_exact_action_route_index(
    ...
    return command_demo_readiness_exact_action_route_index(specs, launcher_argv)


def command_mvp_demo_readiness_exact_action_route_for_engine_action(
    engine_action: str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandDemoReadinessExactActionRouteEntry | None:
    return command_demo_readiness_exact_action_route_for_engine_action(
        engine_action,
        specs,
        launcher_argv,
    )
...
def command_mvp_demo_readiness_action_matrix_contract(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandDemoReadinessActionMatrixContract:
    return command_demo_readiness_action_matrix_contract(specs, launcher_argv)


def command_mvp_demo_readiness_action_matrix_summary(
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[tuple[int, str, str, str, tuple[str, ...], tuple[tuple[str, str], ...]], ...]:
...
def command_mvp_demo_readiness_cli_exact_action_entry_for_argv(
    argv: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> CommandDemoReadinessExactActionEntry | None:
    return command_demo_readiness_cli_exact_action_entry_for_argv(argv, specs, launcher_argv)
...
def command_mvp_demo_readiness_primary_engine_action_for_argv(
    argv: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> str | None:
    return command_demo_readiness_primary_engine_action_for_argv(argv, specs, launcher_argv)


def command_mvp_demo_readiness_primary_action_line_for_argv(
    argv: Sequence[str] | str,
    specs: tuple[CommandSpec, ...] = COMMAND_SPECS,
    launcher_argv: tuple[str, ...] = COMMAND_SMOKE_CLI_LAUNCHER_ARGV,
) -> tuple[str, str] | None:
    return command_demo_readiness_primary_action_line_for_argv(argv, specs, launcher_argv)
