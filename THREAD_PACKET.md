...
- Packet refresh role: `reviewer-fix resubmission refresh 12`
...
- Canonical demo-path step(s) advanced: the CLI-backed MVP loop entry and handoff surface, specifically deterministic `project-open`, `retrieval`, `patch-review`, and `export-handoff` command entrypoints in the active AGENTS demo path.
- Concrete canonical mapping sentence: this slice makes the existing CLI-backed MVP loop more real by forcing the parser-backed `project-open`, `retrieval`, `patch-review`, and `export-handoff` entrypoints to stay aligned with the canonical catalog, so the operator starts and completes the loop through deterministic command contracts rather than drift-prone parser aliases.
...
- Causal link: the Milestone 3 CLI-first loop cannot be run reliably unless the canonical `project-open`, `retrieval`, `patch-review`, and `export-handoff` entrypoints still resolve through the canonical command surface while Textual remains disabled.
- Downstream CLI preservation sentence: because Textual remains disabled, preserving deterministic compatibility across `project-open`, `retrieval`, `patch-review`, and `export-handoff` keeps the full CLI-backed loop drift-resistant instead of only protecting the first gateway.
- AGENTS.md alignment note: this packet explicitly names the exact canonical CLI-backed demo-path surfaces protected by the reviewed slice and ties the claim to the current AGENTS handoff packet and checkpoint requirements.
- Required mapping statement: this `feat-commands` slice is not claiming new workflow reachability; it is migration-safe compatibility hardening for the existing command catalog because `command_cli_contract()` now fails fast when parser/catalog drift would otherwise silently change the operator-facing CLI contract through alias substitution, missing canonical entrypoints, reordered grouped entrypoints, or extra parser-only aliases across the canonical `project-open`, `retrieval`, `patch-review`, and `export-handoff` loop surfaces, preserving the Milestone 3 CLI compatibility requirement while Textual remains disabled.
- Step-1 strengthening sentence: deterministic CLI contract validation makes the CLI-backed MVP loop more real because the exact parser-backed entrypoints the operator uses to open project or document state, run retrieval, review a patch, and hand off exports are now forced to match the canonical catalog before the CLI loop begins.
- Demo-path sentence: this change makes the existing CLI path safer to rely on because the concrete parser-backed command entrypoints an operator already uses for `project-open`, `retrieval`, `patch-review`, and `export-handoff` can no longer silently drift away from the canonical catalog through alias-level or ordering changes.
...
- Reviewer-fix closure note: this refresh exists specifically to satisfy the reviewer-requested handoff fields by stating the exact canonical CLI-backed demo-path steps advanced, tying deterministic `project-open`, `retrieval`, `patch-review`, and `export-handoff` entrypoints to the active Milestone 3 CLI-first loop, and keeping the resubmission scoped to packet alignment rather than new implementation work.
...
- Current revalidation note: this fixer pass re-read the reviewer packet from `fixer__feat-commands__20260424T182743Z.prompt.txt`, refreshed the live handoff metadata so it explicitly maps this slice to deterministic `project-open`, `retrieval`, `patch-review`, and `export-handoff` entrypoints, reran the full required gate set on the current branch tip, and reissued the packet as a fresh metadata-only resubmission commit for re-review traceability.

## Thread Kickoff (High-Risk)

- Branch: `codex/feat-commands`
- Lane/owned paths: `src/qual/commands/**`
- Scope goal: keep the existing Milestone 3 CLI gateway deterministic by locking the parser-backed command surface for `project-open`, `retrieval`, `patch-review`, and `export-handoff` to the canonical catalog without adding new commands or new engine behavior.
- Risk reason: this touches a public command contract in `src/qual/commands/catalog.py` and the explicitly approved shared regression path `tests/unit/test_commands_catalog.py`, so parser drift at canonical `project-open`, `retrieval`, `patch-review`, or `export-handoff` entrypoints would directly weaken the active Milestone 3 CLI-backed loop.
...
3. Refresh the handoff packet so it explicitly names deterministic `project-open`, `retrieval`, `patch-review`, and `export-handoff` entrypoints as the canonical CLI-backed MVP loop surface protected by this contract, explains why the work is migration-safe compatibility hardening rather than second-order cleanup, and keeps the high-risk rationale explicit.
...
- Ready for handoff: this packet now carries the reviewer-requested exact demo-path mapping using the current Milestone 3 and AGENTS wording, explicitly names deterministic `project-open`, `retrieval`, `patch-review`, and `export-handoff` entrypoints as the protected CLI-backed loop surface, and records the migration-safe compatibility justification without implying any broader lane scope.
