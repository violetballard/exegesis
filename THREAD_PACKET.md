...
- Scope goal: Version fallback card debug metadata in `src/qual/ui/a2ui.py` so fallback cards expose a stable contract marker.
- Scope completed: Added `contract_version: 2` to fallback debug metadata in `src/qual/ui/a2ui.py` and updated the fallback card validation logic to require that versioned marker.
- Tasks completed:
  ...
  2. Updated the fallback-card assertions in `tests/unit/test_a2ui_contract.py` so the generic and unknown fallback paths verify the versioned debug metadata and its terminal rendering.
- Files changed:
  - `src/qual/ui/a2ui.py`
...
- Roadmap item(s) affected:
  ...
  - Milestone 5: A2UI Presentation Layer - keep fallback validation aligned with the versioned debug payload exposed by the contract.
