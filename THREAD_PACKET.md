...
This source-bearing fixer pass modifies `src/qual/engine/retrieval/payload.py`, `tests/unit/test_unified_retrieval.py`, and `THREAD_PACKET.md` so sparse excerpt-citation snapshots canonicalize valid padded or case-varied FTS basket item IDs before excerpt-bundle rehydration. Stale PageIndex/embedding-shaped or mismatched IDs still fail closed, while valid FTS citation refs remain basket-promotable without downstream consumers reconstructing identity from raw excerpt IDs.

This source-bearing fixer pass modifies `src/qual/retrieval/service.py`, `tests/unit/test_unified_retrieval.py`, and `THREAD_PACKET.md` so downstream payload and excerpt-bundle `excerpt_hits` carry canonical FTS basket item IDs and singleton basket identity lists in both the hit snapshot and nested provenance. Context-basket consumers can now audit raw excerpt-hit evidence against promotable basket refs without reconstructing `retrieval:fts:<excerpt_id>` identity from the raw excerpt ID.
...
This source-bearing fixer pass modifies `src/qual/retrieval/service.py`, `src/qual/engine/retrieval/payload.py`, `tests/unit/test_unified_retrieval.py`, and `THREAD_PACKET.md` so direct and sparse-rehydrated FTS basket-promotion item snapshots always carry deterministic `promotion_item_fingerprint` values and stale promotion-bundle fingerprints are recomputed from normalized promotion-item plus canonical basket-promotion item fingerprints. Direct promotion-bundle items now use the same fingerprint helper as basket-promotion items, so context-basket consumers can audit promotion-ready snapshots without deriving a missing or stale promotion fingerprint from basket identity fields or widening beyond FTS-first retrieval.

This source-bearing fixer pass modifies `src/qual/retrieval/service.py`, `src/qual/engine/retrieval/payload.py`, `tests/unit/test_unified_retrieval.py`, and `THREAD_PACKET.md` so downstream payload and excerpt-bundle `excerpt_hits` expose the canonical FTS basket-promotion item snapshot, singleton basket-promotion item list, and deterministic `promotion_item_fingerprint` on both top-level hit snapshots and nested provenance. Excerpt-hit-only sparse bundle rehydration now also finalizes deterministic promotion fingerprints and canonical demo-path markers when richer basket-promotion surfaces have been stripped. Context-basket consumers can now audit raw excerpt-hit evidence directly against promotion-ready FTS basket evidence without deriving the promotion fingerprint from basket identity fields or widening beyond FTS-first retrieval.
...
This finalization pass also normalizes sparse `basket_promotion_items` query-constraint snapshots, citation status, and retrieval policy before recomputing canonical basket item and promotion item fingerprints. Context/source bundle rehydration now rejects stale item fingerprints when item-level constraints are normalized, and drops malformed non-object sparse promotion entries, keeping basket promotion evidence deterministic for the engine loop.
...
49. Added deterministic `promotion_item_fingerprint` values to direct and sparse-rehydrated FTS basket-promotion item snapshots, with shared regression coverage proving canonical basket item snapshots no longer omit the promotion fingerprint.
50. Recomputed promotion-bundle fingerprints from normalized promotion items plus canonical basket-promotion item fingerprints, with shared regression coverage proving stale bundle fingerprints are replaced after sparse bundle normalization.
...
- `python -m compileall -q src/qual/retrieval/service.py tests/unit/test_unified_retrieval.py` - passed after direct excerpt-hit promotion evidence was added.
- `PYTHONPATH=. pytest tests/unit/test_unified_retrieval.py -k "basket_promotion_items_backfill_query_context_from_bundle or basket_promotion_bundle_normalizes_query_constraints_snapshot" -q` - passed 2 focused retrieval regressions with 102 deselected after excerpt-hit promotion fingerprints were bound to canonical basket-promotion items.
- `PYTHONPATH=. pytest tests/unit/test_unified_retrieval.py -q` - passed 104 unified retrieval tests and 96 subtests after excerpt-hit promotion evidence was added.
- `./quality-format.sh --check` - passed after this excerpt-hit promotion evidence finalization.
- `./quality-lint.sh` - passed shell syntax and trailing whitespace checks after this excerpt-hit promotion evidence finalization.
- `./quality-test.sh` - passed smoke tests and 487 unit tests, including all unified retrieval tests, after this excerpt-hit promotion evidence finalization.
- `./typecheck-test.sh` - passed Python source compilation under `src/` after this excerpt-hit promotion evidence finalization.
- `make ci` - blocked at scope-check because `tests/unit/test_unified_retrieval.py` is the approved shared regression path; rerun with `SCOPE_ALLOW_SHARED=1`.
- `SCOPE_ALLOW_SHARED=1 make ci` - passed scope-check, format, lint, compile/typecheck, smoke tests, and all unified retrieval coverage, then failed with 6 unrelated sandbox/control-plane `PermissionError` errors when inherited shared-scope test execution attempted to write `.codex/packet_router/logs/fixer__feat-commands__20260513T041012Z.log`, write `.codex/feature_runner/state.json`, invoke `ps`, write `.codex/packet_planner/state.json` in two planner tests, or move recovery artifacts under `.codex/worktree_recovery/feat-a__git-local__20260513T041017Z`.
- `./quality-format.sh --check` - passed after this handoff packet update.
- `./quality-lint.sh` - passed shell syntax and trailing whitespace checks after this handoff packet update.

- `PYTHONPATH=. pytest tests/unit/test_unified_retrieval.py -k "basket_promotion_items_backfill_query_context_from_bundle or basket_promotion_bundle_normalizes_query_constraints_and_recomputes_item_fingerprints" -q` - passed 1 focused retrieval regression with 103 deselected after promotion-bundle fingerprints were bound to normalized basket-promotion item fingerprints.
- `PYTHONPATH=. pytest tests/unit/test_unified_retrieval.py -q` - passed 104 unified retrieval tests and 96 subtests after promotion-bundle fingerprint recomputation was tightened.
- `python -m compileall -q src/qual/retrieval/service.py src/qual/engine/retrieval/payload.py tests/unit/test_unified_retrieval.py` - passed after promotion-bundle fingerprint recomputation was tightened.
- `./quality-format.sh --check` - passed after this basket-promotion item and bundle fingerprint finalization.
- `./quality-lint.sh` - passed shell syntax and trailing whitespace checks after this basket-promotion item and bundle fingerprint finalization.
- `./quality-test.sh` - passed smoke tests and 487 unit tests, including all unified retrieval tests, after this basket-promotion item and bundle fingerprint finalization.
- `./typecheck-test.sh` - passed Python source compilation under `src/` after this basket-promotion item and bundle fingerprint finalization.
- `make ci` - blocked at scope-check because `tests/unit/test_unified_retrieval.py` is the approved shared regression path; rerun with `SCOPE_ALLOW_SHARED=1`.
- `SCOPE_ALLOW_SHARED=1 make ci` - passed scope-check, format, lint, compile/typecheck, smoke tests, and all unified retrieval coverage, then failed with 6 unrelated sandbox/control-plane `PermissionError` errors when inherited shared-scope test execution attempted to write `.codex/packet_router/logs/fixer__feat-commands__20260513T040521Z.log`, write `.codex/feature_runner/state.json`, invoke `ps`, write `.codex/packet_planner/state.json` in two planner tests, or move recovery artifacts under `.codex/worktree_recovery/feat-a__git-local__20260513T040526Z`.
- `pytest tests/unit/test_unified_retrieval.py -k "basket_promotion_items_backfill_query_context_from_bundle"` - blocked during collection because direct pytest without repo-root `PYTHONPATH` could not import `src`.
- `PYTHONPATH=. pytest tests/unit/test_unified_retrieval.py -k "basket_promotion_items_backfill_query_context_from_bundle"` - passed 1 focused retrieval test with 103 deselected after adding direct basket-promotion item fingerprints.
- `python -m compileall -q src/qual/retrieval/service.py src/qual/engine/retrieval/payload.py tests/unit/test_unified_retrieval.py` - passed after adding basket-promotion item fingerprint finalization.
- `PYTHONPATH=. pytest tests/unit/test_unified_retrieval.py -q` - initially failed 7 source/context bundle equality regressions because retrieval evidence basket items did not yet carry the new promotion fingerprint; passed 104 unified retrieval tests and 96 subtests after applying the same fingerprinting to evidence-built basket items.
- `./quality-format.sh --check` - passed after the basket-promotion item fingerprint finalization.
- `./quality-lint.sh` - passed shell syntax and trailing whitespace checks after the basket-promotion item fingerprint finalization.
- `./quality-test.sh` - passed smoke tests and 487 unit tests, including all unified retrieval tests, after the basket-promotion item fingerprint finalization.
- `./typecheck-test.sh` - passed Python source compilation under `src/` after the basket-promotion item fingerprint finalization.
- `make ci` - blocked at scope-check because `tests/unit/test_unified_retrieval.py` is the approved shared regression path; rerun with `SCOPE_ALLOW_SHARED=1`.
- `SCOPE_ALLOW_SHARED=1 make ci` - passed scope-check, format, lint, compile/typecheck, smoke tests, and all unified retrieval coverage, then failed with 6 unrelated sandbox/control-plane `PermissionError` errors when inherited shared-scope test execution attempted to write `.codex/packet_router/logs/fixer__feat-commands__20260513T035519Z.log`, write `.codex/feature_runner/state.json`, invoke `ps`, write `.codex/packet_planner/state.json` in two planner tests, or move recovery artifacts under `.codex/worktree_recovery/feat-a__git-local__20260513T035524Z`.
...
- `./quality-test.sh` - passed smoke tests and 487 unit tests, including all unified retrieval tests, after the sparse basket-promotion fingerprint finalization.
...
- `SCOPE_ALLOW_SHARED=1 make ci` - passed scope-check, format, lint, compile/typecheck, smoke tests, and all unified retrieval tests, then failed with 6 unrelated sandbox/control-plane errors when broader control-plane tests attempted to write `.codex/packet_router/logs/fixer__feat-commands__20260513T031437Z.log`, write `.codex/feature_runner/state.json`, invoke `ps`, write `.codex/packet_planner/state.json` in two planner tests, and move recovery artifacts under `.codex/worktree_recovery/feat-a__git-local__20260513T031442Z`.
- `./quality-format.sh --check` - passed after this handoff packet update.
- `./quality-lint.sh` - passed shell syntax and trailing whitespace checks after this handoff packet update.

Earlier gate run for the previous sparse excerpt-citation basket identity canonicalization finalization:

- `pytest tests/unit/test_unified_retrieval.py -k "sparse_excerpt_citations_canonicalize_valid_fts_basket_ids or sparse_payload_doc_and_excerpt_bundles_use_citation_fallbacks"` - blocked at collection because the shell environment lacked repo-root imports (`ModuleNotFoundError: No module named 'src'`).
- `PYTHONPATH=. pytest tests/unit/test_unified_retrieval.py -k "sparse_excerpt_citations_canonicalize_valid_fts_basket_ids or sparse_payload_doc_and_excerpt_bundles_use_citation_fallbacks"` - passed 2 focused sparse excerpt-citation regression tests, 102 deselected.
...
- `./quality-format.sh --check` - passed after this handoff packet update.
- `./quality-lint.sh` - passed shell syntax and trailing whitespace checks after this handoff packet update.
- `PYTHONPATH=. pytest tests/unit/test_unified_retrieval.py -k "basket_promotion_items_backfill_query_context_from_bundle" -q` - passed 1 focused retrieval regression with 103 deselected after direct basket-promotion item fingerprints were finalized.
- `python -m compileall -q src/qual/retrieval/service.py src/qual/engine/retrieval/payload.py tests/unit/test_unified_retrieval.py` - passed after promotion-bundle items were switched to the shared promotion fingerprint helper.
- `PYTHONPATH=. pytest tests/unit/test_unified_retrieval.py -q` - passed 104 unified retrieval tests and 96 subtests after the promotion fingerprint helper consistency edit.
- `./quality-format.sh --check` - passed after this basket-promotion item fingerprint finalization.
- `./quality-lint.sh` - passed shell syntax and trailing whitespace checks after this basket-promotion item fingerprint finalization.
- `./quality-test.sh` - passed smoke tests and 487 unit tests, including all unified retrieval tests, after this basket-promotion item fingerprint finalization.
- `./typecheck-test.sh` - passed Python source compilation under `src/` after this basket-promotion item fingerprint finalization.
- `make ci` - blocked at scope-check because `tests/unit/test_unified_retrieval.py` is an approved shared regression path; rerun with `SCOPE_ALLOW_SHARED=1`.
...
- `python3 -m unittest tests.unit.test_unified_retrieval.UnifiedRetrievalTests.test_basket_promotion_items_backfill_query_context_from_bundle -q` - passed after adding provenance-backed sparse `title_hint` rehydration coverage, including null top-level title fields.
- `python3 -m unittest tests.unit.test_unified_retrieval -q` - passed 104 unified retrieval tests after adding provenance-backed sparse `title_hint` rehydration.
- `./quality-format.sh --check` - passed after this source-bearing sparse title-provenance edit.
- `./quality-lint.sh` - passed shell syntax and trailing whitespace checks after this source-bearing sparse title-provenance edit.
- `./quality-test.sh` - passed smoke tests and 487 unit tests, including all 104 unified retrieval tests, after this source-bearing sparse title-provenance edit.
- `./typecheck-test.sh` - passed Python source compilation under `src/`.
- `make ci` - blocked at scope-check because `tests/unit/test_unified_retrieval.py` is an approved shared regression path; rerun requires the approved shared-file scope override.
- `SCOPE_ALLOW_SHARED=1 make ci` - passed scope-check, format, lint, compile/typecheck, smoke tests, and retrieval tests, then failed with 6 unrelated sandbox/control-plane `PermissionError` errors when inherited shared-scope test execution attempted to write `.codex/packet_router/logs/fixer__feat-commands__20260513T042812Z.log`, write `.codex/feature_runner/state.json`, invoke `ps`, write `.codex/packet_planner/state.json`, or move recovery artifacts under `.codex/worktree_recovery/feat-a__git-local__20260513T042817Z`.
- `pytest tests/unit/test_unified_retrieval.py -q` - blocked at collection because the shell environment lacked repo-root imports (`ModuleNotFoundError: No module named 'src'`).
- `PYTHONPATH=. pytest tests/unit/test_unified_retrieval.py -q` - passed 104 unified retrieval tests and 96 subtests after safe title provenance was wired through direct FTS retrieval, direct FTS excerpt lookup, and excerpt-hit-only sparse basket rehydration.
- `./quality-format.sh --check` - passed after safe title-provenance finalization.
- `./quality-lint.sh` - passed shell syntax and trailing whitespace checks after safe title-provenance finalization.
- `./quality-test.sh` - passed smoke tests and 487 unit tests, including all 104 unified retrieval tests, after safe title-provenance finalization.
- `./typecheck-test.sh` - passed Python source compilation under `src/` after safe title-provenance finalization.
- `make ci` - blocked at scope-check because `tests/unit/test_unified_retrieval.py` is an approved shared regression path; rerun requires the approved shared-file scope override.
- `SCOPE_ALLOW_SHARED=1 make ci` - passed scope-check, format, lint, compile/typecheck, smoke, and unified retrieval coverage before failing with 6 unrelated sandbox/control-plane `PermissionError` errors when inherited shared-scope test execution attempted to write `.codex/packet_router/logs/fixer__feat-commands__20260513T043227Z.log`, write `.codex/feature_runner/state.json`, invoke `ps`, write `.codex/packet_planner/state.json`, or move recovery artifacts under `.codex/worktree_recovery/feat-a__git-local__20260513T043232Z`.
...
- `PYTHONPATH=. pytest tests/unit/test_unified_retrieval.py -q` - passed 104 unified retrieval tests and 96 subtests after provenance-backed safe `doc_id` and `title_hint` rehydration.
- `./quality-format.sh --check` - passed after document identity provenance finalization.
- `./quality-lint.sh` - passed shell syntax and trailing whitespace checks after document identity provenance finalization.
- `./quality-test.sh` - initially failed once in unrelated `test_repair_shadow_gitdir_repoints_worktree_and_preserves_backup` because a generated `.codex/worktree_recovery/feat-a__git-local__20260513T044431Z` destination from the same timestamp already existed; immediate retry passed smoke tests and 487 unit tests, including all unified retrieval tests.
- `./typecheck-test.sh` - passed Python source compilation under `src/` after document identity provenance finalization.
- `make ci` - blocked at scope-check because `tests/unit/test_unified_retrieval.py` is an approved shared regression path; rerun requires the approved shared-file scope override.
