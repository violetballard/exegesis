...
- reviewer-fix satisfaction note:
  ...
  - required fix 1 is satisfied by naming the `open project/document` operator contract explicitly and explaining how parser/catalog drift would otherwise mutate that operator entrypoint before it reaches the engine path
  ...
  - required fix 3 is satisfied by the live parser-surface drift regressions now present in `tests/unit/test_commands_catalog.py`, including cases where canonical names stay stable but accepted entrypoints drift, and by the plain-checkout `python -m unittest tests.unit.test_commands_catalog` verification pass
