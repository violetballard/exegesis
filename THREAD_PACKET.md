...
- Scope goal: Canonicalize materialized A2UI action order in `src/qual/ui/a2ui.py` so CLI fallback rendering stays deterministic, with matching assertions in `tests/unit/test_a2ui_contract.py`. This maps to `ROADMAP.md` Milestone 5: A2UI Presentation Layer - provide CLI rendering fallback for the same structured payloads, and `PRODUCT_VISION.md` Capability 5: Agent-to-UI protocol (`A2UI`) - agent emits structured presentation artifacts that remain consumable by CLI first.
