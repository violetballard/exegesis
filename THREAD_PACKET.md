...
- Per-task canonical demo-path mapping using current engine-first MVP language:
  ...
  4. The `smoke_tokens` contract fields and `command_flow_smoke_tokens()` / `command_demo_flow_smoke_tokens()` / `command_mvp_flow_smoke_tokens()` helpers advance `preview/apply/reject patch` by exposing the exact first-token CLI smoke route for `diff-preview` / `patch-review`, with the same deterministic route list covering `open project/document`, `retrieve relevant material`, `promote/gather context`, and `save and continue`.
  5. The branch-tip command-catalog target advances `preview/apply/reject patch` via `diff-preview` / `patch-review` by keeping the reviewed runtime surface focused on command-surface stability for the patch-preview CLI fallback path.
  6. Packet refresh work advances the handoff evidence for `open project/document`, `retrieve relevant material`, `promote/gather context`, `preview/apply/reject patch`, and `save and continue` by making the command-surface evidence, review target, and over-budget status explicit for those CLI routes.
