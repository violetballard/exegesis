...
- Packet update note: this commit tightens sparse retrieval basket-promotion fingerprint pairing, the shared retrieval regression surface, and `THREAD_PACKET.md` so sparse fingerprint fallbacks stay attached to the surviving canonical `retrieval:fts:<excerpt_id>` basket identity while stale PageIndex/embedding-shaped refs still fail closed; the final branch tip SHA is reported in the fixer final response.
- Current pass role: source-bearing sparse retrieval basket fingerprint pairing and compact fallback finalization for context-basket promotion.
