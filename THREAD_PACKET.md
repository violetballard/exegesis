...
- Packet update note: this commit refreshes sparse excerpt-citation normalization so valid padded or case-varied FTS basket item IDs canonicalize to `retrieval:fts:<excerpt_id>` before excerpt-bundle rehydration, while stale non-FTS IDs still fail closed; the final branch tip SHA is reported in the fixer final response.
- Current pass role: source-bearing sparse excerpt-citation basket identity canonicalization finalization for context-basket evidence.
...
Current source-bearing sparse excerpt-citation basket identity canonicalization surface in this pass:
...
- Size accounting before this packet refresh: the corrected source-bearing range `378cf9a74a3658058079a32f186fcd254c4a4034..HEAD` already exceeds the high-risk `<=300 net LOC` limit; this pass adds a small sparse excerpt-citation basket identity canonicalization finalization, keeping the range above the limit.
...
Required gates for this corrected merge candidate were re-run on 2026-05-13 against branch `codex/feat-retrieval-fts` after this source-bearing sparse excerpt-citation basket identity canonicalization finalization.
...
- `PYTHONPATH=. pytest tests/unit/test_unified_retrieval.py -q` - passed all 104 unified retrieval tests and 96 subtests after sparse excerpt citations started canonicalizing valid padded or case-varied FTS basket item IDs before excerpt-bundle rehydration.
- `./quality-format.sh --check` - passed after the sparse excerpt-citation basket identity canonicalization finalization.
- `./quality-lint.sh` - passed shell syntax and trailing whitespace checks after the sparse excerpt-citation basket identity canonicalization finalization.
- `./quality-test.sh` - passed smoke tests and 487 unit tests, including all 104 unified retrieval tests, after the sparse excerpt-citation basket identity canonicalization finalization.
