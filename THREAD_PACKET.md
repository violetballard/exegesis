...
- Verified branch head before this fix commit: `bf503a54b80e20c592029fd24e287a64b91c4473`
...
- Removed the off-scope `scripts/scope-check.sh` policy change from this lane so the submitted branch delta is limited to the command file, the reviewer-required shared regression test, and this packet.
...
3. Removed the off-scope `scripts/scope-check.sh` delta so the lane no longer carries repository policy enforcement changes.
...
- Approved shared regression test note: `tests/unit/test_diff_preview.py` is included only to satisfy the reviewer-required regression coverage for the submitted `diff_preview` contract change.
- Ownership note: no tooling/policy file changes remain in this lane; any future `scripts/scope-check.sh` change must ship separately with its own approval and roadmap/vision mapping.
