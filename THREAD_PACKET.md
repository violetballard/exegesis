...
## Risks / blockers
- Risk: `LOW`
- Blockers: local `scope-check`/`make ci` still reject the reviewer-required shared test file on `codex/feat-commands`; the current allowlist only recognizes a shared test exception for `codex/feat-context-storage*`
