...
- Scope goal: Canonicalize materialized A2UI action order in `src/qual/ui/a2ui.py` so CLI fallback rendering stays deterministic, with matching assertions in `tests/unit/test_a2ui_contract.py`. This handoff explicitly maps to `ROADMAP.md` Milestone 5: A2UI Presentation Layer (In Progress) -> scope bullet `Add agent-side card/section/action payload generation with deterministic schemas` and exit criteria `A2UI schema/versioning is documented and stable`, plus `PRODUCT_VISION.md` Capability 5: Agent-to-UI protocol (`A2UI`) for auditability.
