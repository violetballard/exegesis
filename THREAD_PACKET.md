...
- Required mapping statement: this slice directly makes `patch-review` more real in the current CLI-first MVP loop because `command_cli_contract()` in `src/qual/commands/catalog.py` now emits the validated canonical parser-entrypoint projection for the operator-facing `diff-preview` and `diff` surface, and `tests/unit/test_commands_catalog.py` covers both the catalog-order expectation and the parser-drift rejection path so the live patch-review contract fails fast instead of silently changing while Textual remains disabled.
...
- `ROADMAP.md` canonical demo path: the concrete operator step protected is `patch-review` in `project-open -> retrieval -> patch-review -> apply/reject -> persist -> export-handoff`, because `command_cli_contract()` keeps the `diff-preview` and `diff` CLI entrypoints tied to the canonical command catalog and the regression coverage in `tests/unit/test_commands_catalog.py` proves that patch-review now fails fast on drift instead of silently changing behavior.
