...
- Gate evidence note: the files listed above are the corrected full `codex/integrator...HEAD` branch delta after this fix commit removes the out-of-lane `scripts/scope-check.sh` edit. Plain `make scope-check` and `make ci` were run on the pre-commit working tree and passed because feature lanes use `recent` scope mode against the current `HEAD`.
