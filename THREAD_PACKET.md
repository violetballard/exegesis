...
- Merge candidate for re-review: branch tip `HEAD` on `codex/feat-retrieval-fts` after this candidate-resolution fixer commit; the exact final SHA is reported in the fixer deliverable.
...
- `tests/unit/test_unified_retrieval.py` - verifies canonical FTS excerpt lookup, facade exports, deterministic payload/provenance normalization, basket metadata, sparse non-FTS excerpt rejection, sparse duplicate/empty basket reference normalization, unknown doc-scope rejection, unresolved collection-scope rejection, and payload reconstruction.
...
- `./quality-test.sh` - passed; ran smoke plus 137 unit tests, including unified retrieval and `test_unknown_doc_scope_is_rejected_before_retrieval_evidence`.
- `./typecheck-test.sh` - passed; compiled Python sources in `src/`.
- `make ci` - passed; reran setup, scope-check, format, lint, typecheck, and quality tests, including smoke plus 137 unit tests.
