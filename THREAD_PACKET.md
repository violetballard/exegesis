...
- Current fixer commit: branch-tip implementation refresh for direct FTS excerpt lookup audit promotion-source metadata; final SHA is reported in the fixer response after commit.
...
- `src/qual/retrieval/service.py` - implemented FTS-only excerpt fetching, deterministic query/constraint/cache handling, sparse policy reconstruction, final hit re-ranking, explicit FTS strategy aliases, provenance-level `retrieval_source_strategy` aliases, direct excerpt lookup promotion-source metadata, matched-term evidence for basket promotion items, canonical `excerpt_text_hash` audit recording, and `basket_promotion_source` audit recording for excerpt lookups.
