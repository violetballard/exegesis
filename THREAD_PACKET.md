...
- Current fixer pass note: this follow-up pass rechecked the live `src/qual/commands/catalog.py` and `tests/unit/test_commands_catalog.py` state, confirmed the reviewer-requested parser-surface guardrail and alias-drift regressions were already committed, and limited the new work to another handoff metadata refresh plus a fresh required-gates rerun.
