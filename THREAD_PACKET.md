...
- Scope completed: command catalog implementation and regression coverage as represented by the full branch-tip merge diff. This includes the branch-tip `594c21819dfa534f51eb2af784630e21b0acff48` smoke-token API change in `src/qual/commands/catalog.py`; this packet does not claim the implementation basis is narrowed to `f8d860ed9f6299f0169c4f21321ac5f37c949fd3`.
- Implementation review basis: the current branch tip, including `594c21819dfa534f51eb2af784630e21b0acff48`. The implementation history is the full 340-commit set returned by `git log --reverse --format='%H %s' main..HEAD -- src/qual/commands/catalog.py tests/unit/test_commands_catalog.py`.
...
- Per-task canonical demo-path mapping using current engine-first MVP language:
  1. Exact canonical CLI-token validation advances `open project/document` via `bootstrap` / `project-open`, `retrieve relevant material` via `retrieval`, `promote or gather context` via `context-basket`, `preview and apply/reject patch` via `diff-preview` / `patch-review`, and `save and continue` via `terminal` / `export-handoff` by requiring each catalog command to remain present as its exact parser token.
  2. The `_CLI_ENTRYPOINTS` alias-replacement regression advances `preview and apply/reject patch` via `diff-preview` / `patch-review` by proving alias `diff` cannot replace canonical parser token `diff-preview`.
  3. The `_CLI_ENTRYPOINTS` alias-before-canonical regression advances `preview and apply/reject patch` via `diff-preview` / `patch-review` by proving alias `diff` cannot precede canonical parser token `diff-preview`.
  4. The `smoke_tokens` contract fields and `command_flow_smoke_tokens()` / `command_demo_flow_smoke_tokens()` / `command_mvp_flow_smoke_tokens()` helpers advance `preview and apply/reject patch` by exposing the exact first-token CLI smoke route for `diff-preview` / `patch-review`, with the same deterministic route list covering `open project/document`, `retrieve relevant material`, `promote or gather context`, and `save and continue`.
  5. The branch-tip command-catalog target advances `preview and apply/reject patch` via `diff-preview` / `patch-review` by keeping the reviewed runtime surface focused on command-surface stability for the patch-preview CLI fallback path.
  6. Packet refresh work advances the handoff evidence for `open project/document`, `retrieve relevant material`, `promote or gather context`, `preview and apply/reject patch`, and `save and continue` by making the command-surface evidence, review target, and over-budget status explicit for those CLI routes.
...
1. Tightened `command_cli_contract()` so each `command_names()` canonical entry must appear as a canonical CLI token in catalog order. Demo-path mapping: advances `open project/document` (`bootstrap` / `project-open`), `retrieve relevant material` (`retrieval`), `promote or gather context` (`context-basket`), `preview and apply/reject patch` (`diff-preview` / `patch-review`), and `save and continue` (`terminal` / `export-handoff`) CLI entrypoint stability.
2. Added a regression test proving that removing canonical token `diff-preview` while keeping alias `diff` raises `ValueError`. Demo-path mapping: strengthens `preview and apply/reject patch` by preserving the canonical `diff-preview` / `patch-review` parser surface.
3. Added a regression test proving that alias `diff` cannot appear before canonical token `diff-preview`. Demo-path mapping: strengthens `preview and apply/reject patch` by preserving canonical-token precedence for the `diff-preview` / `patch-review` route.
4. Exposed smoke-token contract fields on `CommandSurfaceContract` and `CommandCliRouteContract`, plus `command_flow_smoke_tokens()`, `command_demo_flow_smoke_tokens()`, and `command_mvp_flow_smoke_tokens()`. Demo-path mapping: gives smoke checks an exact deterministic token sequence for `preview and apply/reject patch` (`diff-preview` / `patch-review`) and the adjacent CLI-first MVP routes.
5. Kept the corrected branch-tip merge diff to `src/qual/commands/catalog.py`, `tests/unit/test_commands_catalog.py`, and packet metadata only, while truthfully reporting that the history behind those files is over budget. Demo-path mapping: keeps this lane scoped to preserving the CLI surface for `preview and apply/reject patch`.
6. Regenerated `THREAD_PACKET.md` and `THREAD.md` so they describe the full branch-tip target, classify implementation history by actual file touches, and record this final fixer pass as the review target for `fixer__feat-commands__20260429T234430Z`. Demo-path mapping: keeps handoff evidence aligned with `open project/document`, `retrieve relevant material`, `promote or gather context`, `preview and apply/reject patch`, and `save and continue` CLI routes.
7. Kept the unit test isolated to the catalog module so the corrected target does not require `src/qual/commands/__init__.py`. Demo-path mapping: limits validation to command-surface behavior used by `open project/document`, `retrieve relevant material`, `promote or gather context`, `preview and apply/reject patch`, and `save and continue` in the engine-first CLI fallback path.
...
- `python -m unittest tests.unit.test_commands_catalog`: PASS on 2026-04-29 for `fixer__feat-commands__20260429T234430Z` (`45` tests).
- `make scope-check`: PASS on 2026-04-29 for `fixer__feat-commands__20260429T234430Z`.
- `./quality-format.sh --check`: PASS on 2026-04-29 for `fixer__feat-commands__20260429T234430Z`.
- `./quality-lint.sh`: PASS on 2026-04-29 for `fixer__feat-commands__20260429T234430Z`.
- `./quality-test.sh`: PASS on 2026-04-29 for `fixer__feat-commands__20260429T234430Z` (`116` tests).
- `./typecheck-test.sh`: PASS on 2026-04-29 for `fixer__feat-commands__20260429T234430Z`.
- `make ci`: PASS on 2026-04-29 for `fixer__feat-commands__20260429T234430Z`.
...
This packet asks review of the full corrected branch-tip target: command catalog implementation through `594c21819dfa534f51eb2af784630e21b0acff48`, its unit tests, and accurate packet metadata. The corrected command-catalog work makes the canonical demo-path step `preview and apply/reject patch` more real by preserving deterministic canonical CLI command names, parser tokens, alias behavior, and smoke-token route metadata for the `diff-preview` / `patch-review` command surface. This is concrete blocker-removal rather than second-order contract cleanup because parser/catalog drift could silently change the CLI smoke route operators use to prove patch preview while the UI remains disabled. The branch tip is over the normal AGENTS size budget and must be accepted explicitly or split by integrator instruction.
