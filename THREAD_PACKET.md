...
- Traceability correction: the current merge candidate is branch tip `HEAD`, not `adfa8cdadd43747ffbcb612e4151e262b13e52ca`, not `3fa8b5edfd7dac6eb22715eeed64ac8a3325ad52`, not `b6e026a0adf4137108b2388b88a630f902ab8b9f`, not `36080d74a5792e5387b3f68e39ff99c41467b191`, not `b8416aba3589ded2e2b0882a8d00a6ad8ee8ab17`, and not the earlier branch tip `519082d6605a4eb1dc2171507769ecee3617767c`. Earlier packet wording about `378cf9a74a3658058079a32f186fcd254c4a4034..adfa8cdadd43747ffbcb612e4151e262b13e52ca` and about post-`adfa8cdadd43747ffbcb612e4151e262b13e52ca` metadata-only commits is superseded. No commit with source or test changes in `adfa8cdadd43747ffbcb612e4151e262b13e52ca..HEAD` is described as metadata-only in this packet.
- Implementation commit classification: all source-bearing commits after `adfa8cdadd43747ffbcb612e4151e262b13e52ca` are included in the reviewer-requested range `adfa8cdadd43747ffbcb612e4151e262b13e52ca..HEAD`. The source/test file set for those commits is `src/qual/engine/retrieval/__init__.py`, `src/qual/engine/retrieval/fts_strategy.py`, `src/qual/engine/retrieval/payload.py`, `src/qual/retrieval/__init__.py`, `src/qual/retrieval/service.py`, and `tests/unit/test_unified_retrieval.py`. Packet-only files in the same historical range are `.codex/kickoff_packets/feat-retrieval-fts.md`, `.codex/lane_meta/feat-retrieval-fts.json`, and `THREAD_PACKET.md`.
- Source-bearing examples called out for re-review: `19b99a7034e28aadba7ddc7ef20fdf9a2fcbae2a` (`Fail closed on unresolved FTS collection scopes`) changes `src/qual/retrieval/service.py` and `tests/unit/test_unified_retrieval.py`; `d00ee5c38a6b8a49ae0c343a97bc11796b1bac81` (`Expose retrieval candidate resolution`) changes `src/qual/retrieval/service.py` and `src/qual/engine/retrieval/payload.py`; `3fa8b5edfd7dac6eb22715eeed64ac8a3325ad52` (`Keep sparse basket items authoritative`) changes `src/qual/engine/retrieval/payload.py` and `tests/unit/test_unified_retrieval.py`; `dc8db42ddd554e38c1d390a089eac26ae8a16aa7` (`Finalize retrieval lookup fingerprints`) changes `src/qual/retrieval/service.py` and `tests/unit/test_unified_retrieval.py`; `b8f8eeab12684e6269c7bb70703370711a40790d` (`Finalize retrieval basket readiness`) changes `src/qual/engine/retrieval/payload.py` and `src/qual/retrieval/service.py`; `b6e026a0adf4137108b2388b88a630f902ab8b9f` (`Expose excerpt lookup fingerprints in citations`) changes `src/qual/retrieval/service.py`, `tests/unit/test_unified_retrieval.py`, and `THREAD_PACKET.md`; `42a407e95e3fd332cfb4d76513b59de57a5c01f6` (`Finalize retrieval excerpt lookup audit`) changes `src/qual/retrieval/service.py`; `b8416aba3589ded2e2b0882a8d00a6ad8ee8ab17` (`Finalize retrieval query date constraints`) changes `src/qual/engine/retrieval/__init__.py`; and `519082d6605a4eb1dc2171507769ecee3617767c` (`Finalize sparse retrieval date ranges`) changes `src/qual/engine/retrieval/payload.py` and `tests/unit/test_unified_retrieval.py`. These are implementation commits when included, not metadata-only commits.
...
This active-policy guard finalization pass also makes sparse engine payload reconstruction fail closed when a snapshot tries to mark a non-FTS backend, mode, or active strategy as authoritative. PageIndex and embeddings can still appear only as deferred strategy identifiers; they cannot become active through a stale or malformed sparse policy bundle.
...
4. Added owned-path candidate-resolution metadata so diagnostics, evidence, audit records, and doc-hit provenance expose the exact final candidate docs searched and whether they came from doc scope or FTS shortlist narrowing, made sparse basket promotion items authoritative over stale summary IDs/counts during reconstruction, carried FTS excerpt lookup fingerprints into retrieval-result promotion refs and citation/evidence snapshots, added explicit basket promotion readiness across retrieval payloads and sparse reconstruction, finalized normalized FTS excerpt lookup audit metadata for basket promotion traceability, required ordered date-range constraints in the canonical engine query constructor and sparse payload reconstruction, made sparse policy reconstruction reject non-FTS active strategies/backends/modes, then refreshed handoff metadata so the reviewed range, file list, gate evidence, and canonical demo-path mapping cover the actual branch-tip merge candidate.
...
Current active-policy guard finalization focused checks:

- `python -m unittest tests.unit.test_unified_retrieval -q` - passed; ran 71 unified retrieval tests after the sparse FTS policy guard.
- `python - <<'PY' ...` sparse non-FTS active policy smoke check - passed; verified sparse source-bundle reconstruction rejects an active `pageindex` strategy with `ValueError`.

Current active-policy guard finalization required gates:
