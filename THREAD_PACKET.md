...
- Verified branch head before this fix commit: `678368713ef7d5ca62b74407c6f33c399a53b77c`
...
- Removed the out-of-lane `scripts/scope-check.sh` policy change from the submitted branch.
- Regenerated this handoff packet from the real `codex/integrator...HEAD` branch delta.
...
3. Removed the out-of-lane `scripts/scope-check.sh` policy change from the submitted branch.
4. Regenerated the feature handoff packet so the submitted branch delta, files-changed statement, and gate outcomes match the corrected branch state.
...
- None. This change affects local `diff_preview` output formatting plus the reviewer-required shared regression test; no routing or provider behavior changed.
