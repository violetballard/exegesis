...
- Roadmap/vision mapping:
  - Roadmap item(s) affected: Milestone 5: A2UI Presentation Layer - define `A2UI` output contract for agent-produced presentation artifacts; provide CLI rendering fallback for the same structured payloads.
  - Vision capability affected: Capability 5: Agent-to-UI protocol (`A2UI`) - agent emits structured presentation artifacts (cards, sections, actions, metadata) that remain consumable by CLI first, then `Exegesis Console`, then future Studio UI.
