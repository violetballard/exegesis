...
- Narrowed `src/qual/commands/diff_preview.py` back to behavior already covered by the existing command tests, removing the unsupported JSON/label/fingerprint contract expansion from this lane handoff.
- Kept the summary-stat helper cleanup in `src/qual/commands/diff_preview.py`.
