...
- Branch head note: this packet is attached to the code-bearing `diff_preview` fix commit, not the later packet-only head.
...
- Shared-file exception note: `tests/unit/test_diff_preview.py` is included as reviewer-required shared regression coverage for the `diff_preview` contract, and the approval note is recorded in this packet.
