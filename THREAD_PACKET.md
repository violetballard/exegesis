...
- Packet update note: this commit adds a stable `fts_match_query_fingerprint` across FTS retrieval diagnostics, provenance, manifest/result identity, evidence, source/context bundle rehydration, and audit metadata without storing plaintext query terms, explicitly binds that value into the result fingerprint payload, then refreshes `THREAD_PACKET.md`; the final branch tip SHA is reported in the fixer final response.
- Current pass role: source-bearing FTS match-query audit fingerprint finalization for retrieval evidence.
...
Current source-bearing FTS match-query fingerprint finalization surface in this pass:
...
- Size accounting before this packet refresh: the corrected source-bearing range `378cf9a74a3658058079a32f186fcd254c4a4034..HEAD` already exceeds the high-risk `<=300 net LOC` limit; this pass adds a small FTS match-query fingerprint finalization and regression update, keeping the range above the limit.
...
Required gates for this corrected merge candidate were re-run on 2026-05-13 against branch `codex/feat-retrieval-fts` after this source-bearing FTS match-query fingerprint finalization.
...
- Required retrieval and local format/lint/typecheck/test gates are green for this pass. Full `make ci` is not green in this sandbox: without `SCOPE_ALLOW_SHARED=1` it stops at the approved shared regression path, and with `SCOPE_ALLOW_SHARED=1` it passes scope-check, format, lint, compile/typecheck, smoke tests, and all unified retrieval tests before failing on unrelated sandbox/control-plane `PermissionError` operations in broader control-plane tests. Retrieval tests pass in both the targeted and full local test gates.
