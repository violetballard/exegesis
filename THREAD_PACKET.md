...
3. Removed the synthetic `tasks_completed` fallback from `codex_packet_handoff/tools/planner.py`, clarified generated feature packets so the active engine execution order cannot be read as a hard merge prerequisite unless a packet explicitly lists one, and hardened planner packet emission to recreate the feature inbox immediately before writing.
   Canonical demo-path step advanced: `preview and apply or reject a patch`; prevents generated handoff packets and stale packet claims from inventing completed work or misleading review of the patch-preview/apply support surface, while keeping fresh packet resubmission from failing on a missing generated packet directory.
...
The isolated runtime change at `b929fe6c7a1159c7882acedd247aca31a93cd123` canonicalized materialized A2UI action order and added focused test coverage in `src/qual/ui/a2ui.py` and `tests/unit/test_a2ui_contract.py`. Current `main` already contains that behavior, so the authoritative review target is the corrected handoff/planner diff in `main..HEAD`, not the historical runtime commit and not the historical `b929fe6c7..HEAD` range. `codex_packet_handoff/tools/planner.py` is changed only to remove the synthetic `tasks_completed` fallback, clarify that engine execution order is planning guidance rather than an implicit merge prerequisite, and ensure missing generated packet inbox directories are recreated before fresh packet writes. Packet-planner regression coverage lives in `tests/unit/test_lane_profiles.py`; `tests/unit/test_packet_planner.py` is not present in this tree and is not claimed as coverage.
