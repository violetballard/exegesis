...
- Packet update note: this commit refreshes excerpt-hit payload/provenance basket identity so canonical FTS `basket_item_id` and singleton `basket_item_ids`/`basket_item_fingerprints` lists travel with the raw excerpt-hit snapshots consumed by downstream payload and excerpt-bundle readers; the final branch tip SHA is reported in the fixer final response.
- Current pass role: source-bearing excerpt-hit basket identity provenance finalization for context-basket evidence.
...
Current source-bearing excerpt-hit basket identity provenance surface in this pass:
