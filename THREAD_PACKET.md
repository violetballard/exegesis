...
- Packet update note: this commit refreshes sparse basket-promotion item normalization so stale query-constraint snapshots, basket item fingerprints, and promotion item fingerprints are recomputed before context/source bundle rehydration, and malformed non-object sparse promotion entries fail closed before reaching basket evidence, then refreshes `THREAD_PACKET.md`; the final branch tip SHA is reported in the fixer final response.
- Current pass role: source-bearing sparse basket-promotion item normalization finalization for context-basket evidence.
...
- Size accounting before this packet refresh: the corrected source-bearing range `378cf9a74a3658058079a32f186fcd254c4a4034..HEAD` already exceeds the high-risk `<=300 net LOC` limit; this pass adds a small sparse basket-promotion item fingerprint normalization finalization, keeping the range above the limit.
...
Required gates for this corrected merge candidate were re-run on 2026-05-13 against branch `codex/feat-retrieval-fts` after this source-bearing sparse basket-promotion item fingerprint normalization finalization.

- `python -m pytest tests/unit/test_unified_retrieval.py -q -k "basket_promotion_bundle_normalizes_query_constraints_snapshot"` - passed 1 focused sparse basket-promotion item normalization regression, 102 deselected, after stale basket item and promotion item fingerprints were recomputed and malformed non-object promotion entries failed closed.
- `python -m pytest tests/unit/test_unified_retrieval.py -q` - passed all 103 unified retrieval tests and 96 subtests after stale basket-promotion item query constraints, basket fingerprints, and promotion fingerprints were normalized before context-bundle rehydration, with malformed non-object promotion entries rejected.
- `./quality-format.sh --check` - passed after the sparse basket-promotion item fingerprint normalization finalization.
- `./quality-lint.sh` - passed shell syntax and trailing whitespace checks after the sparse basket-promotion item fingerprint normalization finalization.
- `./quality-test.sh` - passed smoke tests and 486 unit tests, including all 103 unified retrieval tests, after the sparse basket-promotion item fingerprint normalization finalization.
