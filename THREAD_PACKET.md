...
- Packet update note: this commit updates direct FTS excerpt lookup provenance, the shared retrieval regression surface, and `THREAD_PACKET.md` so canonical excerpt lookup provenance carries the same deterministic basket item ID and fingerprint lists exposed at top level and in audit events; the final branch tip SHA is reported in the fixer final response.
- Current pass role: source-bearing FTS excerpt lookup provenance finalization for auditable context-basket promotion.
...
- Size accounting before this packet refresh: the corrected source-bearing range `378cf9a74a3658058079a32f186fcd254c4a4034..HEAD` already exceeds the high-risk `<=300 net LOC` limit; this pass adds a small excerpt lookup provenance binding and packet update, keeping the range above the limit.
...
Required gates for this corrected merge candidate were re-run on 2026-05-12 against branch `codex/feat-retrieval-fts` after this source-bearing direct excerpt lookup provenance fix.

- `python -m pytest tests/unit/test_unified_retrieval.py -k "fts_excerpt"` - passed 7 focused FTS excerpt lookup regressions after the edit.
...
- `./typecheck-test.sh` - passed Python source compilation under `src/` after the edit.
...
- `./quality-format.sh --check` - passed after the packet refresh.
- `./quality-lint.sh` - passed shell syntax and trailing whitespace checks after the packet refresh.
