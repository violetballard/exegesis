...
1. Required fix 1, regenerate packet from the actual branch tip / merge candidate: satisfied by this packet using the final branch tip after this `2026-04-28T23:10:30Z` fixer pass as the review basis and not classifying commits as metadata-only when they modify `src/qual/commands/catalog.py` or `tests/unit/test_commands_catalog.py`.
2. Required fix 2, include every code-bearing catalog/test commit in the implementation basis: satisfied by reviewing implementation, tests, and packet metadata together from branch tip, with all `src/qual/commands/catalog.py` and `tests/unit/test_commands_catalog.py` changes included.
...
4. Required fix 4, parser-surface drift tests: satisfied at branch tip by focused regressions for added accepted aliases, removed accepted aliases, same-canonical alias substitution including `diff` to `diff_preview`, token reorder, grouped parser drift, lookup-table token substitution, and lookup-table shape/order drift.
...
6. Required fix 6, gate rerun: focused catalog regressions passed at `2026-04-28T23:10:30Z`; all required gates passed at `2026-04-28T23:11:31Z`.
