...
2. Added live parser-surface parity checks to `command_cli_contract()` that compare raw argparse choices, canonical parser projection, catalog CLI tokens, and the lookup table, so same-canonical parser-token drift is rejected.
...
4. Added focused regression coverage in `tests/unit/test_commands_catalog.py` for parser-surface drift and smoke-contract public exports, including tests that patch the real `_build_parser()` choices and intercept `argparse._SubParsersAction.add_parser()` to rewrite the top-level `diff-preview` parser token.
...
- High-risk net LOC budget: exceeded in the actual review range. Pre-fixer accounting for `f8d860ed9..HEAD` was `1969 insertions(+), 109 deletions(-)`, and this docs-only fixer commit adds to the handoff-document count. This is explicitly disclosed for reviewer/integrator handling instead of being presented as a narrow catalog-only slice.
