...
4. Validated date-range constraints and canonical query text/scope constraints at the retrieval boundary, preserved normalized query constraints during sparse citation-bundle rehydration, and added stable FTS excerpt lookup fingerprints to payload/provenance/audit snapshots, with approved shared regression coverage in `tests/unit/test_unified_retrieval.py` for the FTS-first retrieval behavior, payload normalization, cache metadata, facade exports, citation/provenance helpers, excerpt lookup, and date-range validation included in this task group.
...
Additional current finalization diff after `22df380914bce7606c83026492636d763a9e9c97`:

- `M THREAD_PACKET.md` - handoff packet restamp.
- `M src/qual/retrieval/service.py` - adds deterministic `lookup_fingerprint` to FTS excerpt lookup payload/provenance/audit metadata.
- `M tests/unit/test_unified_retrieval.py` - covers stable lookup fingerprints and audit metadata on the canonical FTS excerpt lookup path.
...
Current finalization remains low blast radius: the final fixer commit updates the FTS excerpt lookup payload/provenance/audit fingerprint, the approved shared retrieval regression, and this handoff packet. No integrator-locked file edits are added by this fixer pass.
...
- Canonical demo-path mapping: advances `retrieve relevant material` by preventing invalid date filters and whitespace-only retrieval queries from producing misleading or unstable retrieval evidence, by making the normalized retrieval constraints visible and rehydratable in bundle/provenance/citation snapshots used for basket/context promotion, and by making fetched FTS excerpt payloads auditable through stable lookup fingerprints.
