...
- Packet update note: this commit refreshes direct and sparse excerpt-hit promotion evidence so downstream payload, nested provenance snapshots, and excerpt-hit-only sparse rehydration carry deterministic FTS basket-promotion fingerprints; the final branch tip SHA is reported in the fixer final response.
- Current pass role: source-bearing excerpt-hit promotion evidence finalization for context-basket audit snapshots and sparse excerpt fallback.
...
Required gates for this corrected merge candidate were re-run on 2026-05-13 against branch `codex/feat-retrieval-fts` after this source-bearing excerpt-hit promotion evidence and sparse fallback finalization.

- `python -m compileall -q src/qual/engine/retrieval/payload.py tests/unit/test_unified_retrieval.py` - passed after adding the engine payload promotion-fingerprint helper and excerpt-hit-only sparse fallback regression.
- `PYTHONPATH=. pytest tests/unit/test_unified_retrieval.py -k "basket_promotion_items_backfill_query_context_from_bundle" -q` - initially failed because the new assertion over-claimed byte-identical fingerprints between full basket items and excerpt-hit-only fallback items; passed 1 focused regression with 103 deselected after tightening the assertion to stable deterministic fallback fingerprint behavior.
- `PYTHONPATH=. pytest tests/unit/test_unified_retrieval.py -q` - passed 104 unified retrieval tests and 96 subtests after excerpt-hit-only sparse fallback promotion fingerprints and demo-path markers were finalized.
- `./quality-format.sh --check` - passed after this excerpt-hit promotion evidence and sparse fallback finalization.
- `./quality-lint.sh` - passed shell syntax and trailing whitespace checks after this excerpt-hit promotion evidence and sparse fallback finalization.
- `./quality-test.sh` - passed smoke tests and 487 unit tests, including all unified retrieval tests, after this excerpt-hit promotion evidence and sparse fallback finalization.
- `./typecheck-test.sh` - passed Python source compilation under `src/` after this excerpt-hit promotion evidence and sparse fallback finalization.
