...
- Kept regression coverage focused on the `summary_only` no-diff payload behavior in `tests/unit/test_diff_preview.py`.
...
2. Kept the regression coverage focused on the canonical `summary_only` fingerprint behavior in `tests/unit/test_diff_preview.py`.
