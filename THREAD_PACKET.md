...
- Packet update note: this commit tightens sparse source/context bundle basket promotion item normalization so raw excerpt-only sparse items cannot leak raw excerpt IDs or orphaned fingerprints into context-basket evidence, then refreshes `THREAD_PACKET.md`; the final branch tip SHA is reported in the fixer final response.
- Current pass role: source-bearing raw excerpt-only sparse basket item fail-closed finalization for FTS-first context-basket evidence.
...
Required gates for this corrected merge candidate were re-run on 2026-05-13 against branch `codex/feat-retrieval-fts` after this source-bearing raw excerpt-only sparse basket item fail-closed finalization.
