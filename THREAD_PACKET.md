...
- Packet update note: this commit updates sparse retrieval source/context basket identity rehydration, the shared retrieval regression surface, and `THREAD_PACKET.md` so compact source-bundle snapshots can recover canonical FTS basket item IDs, counts, and fingerprints from the retrieval manifest when top-level and summary basket fields have been stripped; the final branch tip SHA is reported in the fixer final response.
- Current pass role: source-bearing sparse retrieval manifest basket identity finalization for FTS-first context-basket promotion.
