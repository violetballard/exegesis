...
## Current Capability Alignments (2026-03-04)

- `codex/feat-webconsole-ui`: Reinforces capability #4 (operator-first control surface) and #5 (A2UI contract) by giving the SSE terminal Alt+R (reconnect) and Alt+A (auto-retry toggle) shortcuts plus inline hints so keyboard-first operators keep parity with mouse controls.
