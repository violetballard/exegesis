...
from exegesis_engine.audit import AuditLog

if TYPE_CHECKING:
    from exegesis_engine.docindex.service import DocIndexBuildOptions, DocIndexService

from exegesis_engine.retrieval.facade import (
    FTS_FIRST_POLICY,
    FTSStrategy,
...
from exegesis_engine.retrieval.interface import StrategyRun
from exegesis_engine.retrieval.payload import (
    RETRIEVAL_DEMO_PATH_STEPS,
    _basket_promotion_provenance_from_snapshot,
    _context_bundle_fingerprint,
)
...
_FTS_RESULT_BASKET_PROMOTION_SOURCE = "fts_retrieval_result"
_FTS_EXCERPT_LOOKUP_BASKET_PROMOTION_SOURCE = "fts_excerpt_lookup"
_SUPPORTED_FTS_BASKET_PROMOTION_SOURCES = frozenset(
    {
        _FTS_RESULT_BASKET_PROMOTION_SOURCE,
        _FTS_EXCERPT_LOOKUP_BASKET_PROMOTION_SOURCE,
    }
)
_REQUIRED_BASKET_PROMOTION_TEXT_FIELDS = (
    "item_id",
    "basket_item_id",
    "doc_id",
    "excerpt_id",
    ...
    "promotion_item_fingerprint",
    "query_fingerprint",
    "fts_match_query_fingerprint",
    "result_fingerprint",
)
_REQUIRED_DOC_SOURCE_RECORD_TEXT_FIELDS = (
    "doc_id",
    "doc_type",
    "source_type",
    "source_hash",
...
def _source_type_from_provenance(provenance: dict[str, object]) -> str | None:
    return _optional_text(provenance.get("source_type"))
...
def _basket_item_add_kwargs_from_promotion_item(
    promotion_item: dict[str, object],
) -> dict[str, object]:
    for field_name in _REQUIRED_BASKET_PROMOTION_TEXT_FIELDS:
        val = promotion_item.get(field_name)
        if not isinstance(val, str) or not val.strip():
            raise ValueError(f"promotion_item must carry a non-empty {field_name}")
        if field_name not in ("excerpt_text", "snippet"):
            if val != val.strip():
                raise ValueError(f"promotion_item {field_name} must not contain leading or trailing whitespace")
    basket_item_id = cast(str, promotion_item["basket_item_id"])
    if promotion_item["item_id"] != basket_item_id:
        raise ValueError("promotion_item item_id must match basket_item_id")
    expected_basket_item_id = _basket_item_id_for_excerpt(
        source_strategy=promotion_item.get("source_strategy"),
        excerpt_id=promotion_item.get("excerpt_id"),
    )
    if basket_item_id != expected_basket_item_id:
        raise ValueError("promotion_item basket_item_id must match FTS excerpt_id")
    if promotion_item.get("source_strategy") != _FTS_SOURCE_STRATEGY:
        raise ValueError("promotion_item must come from FTS source_strategy")
    if promotion_item.get("retrieval_source_strategy") != _FTS_SOURCE_STRATEGY:
        raise ValueError("promotion_item must carry FTS retrieval_source_strategy")
    if promotion_item.get("retrieval_backend") != "sqlite_fts":
        raise ValueError("promotion_item must carry sqlite_fts retrieval_backend")
    if promotion_item.get("retrieval_mode") != "fts_first":
        raise ValueError("promotion_item must carry fts_first retrieval_mode")
    if promotion_item.get("basket_promotion_source") not in _SUPPORTED_FTS_BASKET_PROMOTION_SOURCES:
        raise ValueError("promotion_item must carry FTS basket_promotion_source")
    if promotion_item.get("item_type") != "excerpt":
        raise ValueError("promotion_item must carry excerpt item_type")
    doc_source_record = promotion_item.get("doc_source_record")
    if not isinstance(doc_source_record, dict) or not doc_source_record:
        raise ValueError("promotion_item must carry a non-empty doc_source_record")
    for field_name in _REQUIRED_DOC_SOURCE_RECORD_TEXT_FIELDS:
        val = doc_source_record.get(field_name)
        if not isinstance(val, str) or not val.strip():
            raise ValueError(f"promotion_item doc_source_record must carry a non-empty {field_name}")
        if val != val.strip():
            raise ValueError(f"promotion_item doc_source_record {field_name} must not contain leading or trailing whitespace")
    embedded_doc_source_fingerprint = cast(str, doc_source_record["doc_source_record_fingerprint"])
    if promotion_item["doc_source_record_fingerprint"] != embedded_doc_source_fingerprint:
        raise ValueError("promotion_item doc_source_record_fingerprint must match doc_source_record")
    expected_doc_source_fingerprint = RetrievalService._stable_fingerprint(
        {
            key: value
            for key, value in doc_source_record.items()
            if key != "doc_source_record_fingerprint"
        }
    )
    if embedded_doc_source_fingerprint != expected_doc_source_fingerprint:
        raise ValueError("promotion_item doc_source_record_fingerprint is stale")
    for field_name in (
        "doc_id",
        "doc_type",
        "source_type",
        ...
        promotion_value = _optional_text(promotion_item.get(field_name))
        if promotion_value is not None and _optional_text(doc_source_record.get(field_name)) != promotion_value:
            raise ValueError(f"promotion_item doc_source_record must match {field_name}")
    excerpt_text = cast(str, promotion_item["excerpt_text"])
    expected_excerpt_text_hash = hashlib.sha256(excerpt_text.encode("utf-8")).hexdigest()
    if promotion_item["excerpt_text_hash"] != expected_excerpt_text_hash:
        raise ValueError("promotion_item excerpt_text_hash is stale")
    hash_alias = _optional_text(promotion_item.get("hash"))
    if hash_alias is not None and hash_alias != expected_excerpt_text_hash:
        raise ValueError("promotion_item hash must match excerpt_text_hash")
    snippet = cast(str, promotion_item["snippet"])
    expected_snippet_text_hash = hashlib.sha256(snippet.encode("utf-8")).hexdigest()
    if promotion_item["snippet_text_hash"] != expected_snippet_text_hash:
        raise ValueError("promotion_item snippet_text_hash is stale")
    canonical_span = RetrievalService._canonicalize_span(promotion_item.get("span"))
    if canonical_span is None:
        raise ValueError("promotion_item must carry a valid span")
    expected_excerpt_fingerprint = RetrievalService._build_excerpt_fingerprint(
        doc_id=cast(str, promotion_item["doc_id"]),
        excerpt_id=cast(str, promotion_item["excerpt_id"]),
        span=canonical_span,
        text_hash=expected_excerpt_text_hash,
        source_hash=cast(str, promotion_item["source_hash"]),
    )
    if promotion_item["excerpt_fingerprint"] != expected_excerpt_fingerprint:
        raise ValueError("promotion_item excerpt_fingerprint is stale")
    expected_excerpt_lookup_fingerprint = RetrievalService._build_excerpt_lookup_fingerprint(
        excerpt_id=cast(str, promotion_item["excerpt_id"]),
        doc_id=cast(str, promotion_item["doc_id"]),
        source_hash=cast(str, promotion_item["source_hash"]),
        span=canonical_span,
    ...
    if promotion_item["excerpt_lookup_fingerprint"] != expected_excerpt_lookup_fingerprint:
        raise ValueError("promotion_item excerpt_lookup_fingerprint is stale")
    query_constraints = promotion_item.get("query_constraints")
    query_constraints_fingerprint = _optional_text(
        promotion_item.get("query_constraints_fingerprint")
    )
    if query_constraints is not None or query_constraints_fingerprint is not None:
        if not isinstance(query_constraints, dict):
            raise ValueError("promotion_item must carry query_constraints for query_constraints_fingerprint")
        expected_query_constraints_fingerprint = RetrievalService._stable_fingerprint(query_constraints)
        if query_constraints_fingerprint is None:
            raise ValueError("promotion_item must carry query_constraints_fingerprint")
        if query_constraints_fingerprint != expected_query_constraints_fingerprint:
            raise ValueError("promotion_item query_constraints_fingerprint is stale")
    expected_basket_item_fingerprint = RetrievalService._basket_item_fingerprint(promotion_item)
    if promotion_item["basket_item_fingerprint"] != expected_basket_item_fingerprint:
        raise ValueError("promotion_item basket_item_fingerprint is stale")
    basket_item_ids = promotion_item.get("basket_item_ids")
    if basket_item_ids is not None and (
        not isinstance(basket_item_ids, list) or basket_item_id not in basket_item_ids
    ):
        raise ValueError("promotion_item basket_item_ids must include basket_item_id")
    basket_item_fingerprints = promotion_item.get("basket_item_fingerprints")
    if (
        not isinstance(basket_item_fingerprints, list)
        or expected_basket_item_fingerprint not in basket_item_fingerprints
    ):
        raise ValueError("promotion_item basket_item_fingerprints must include basket_item_fingerprint")
    expected_promotion_item_fingerprint = RetrievalService._stable_fingerprint(
        {
            key: value
            for key, value in promotion_item.items()
            if key != "promotion_item_fingerprint"
        }
    )
    if promotion_item["promotion_item_fingerprint"] != expected_promotion_item_fingerprint:
        raise ValueError("promotion_item promotion_item_fingerprint is stale")
    title_hint = promotion_item.get("title_hint")
    label = title_hint if isinstance(title_hint, str) and title_hint.strip() else basket_item_id
    return {
        "item_id": basket_item_id,
        "item_type": str(promotion_item.get("item_type") or "excerpt"),
        "label": label,
        "payload": copy.deepcopy(promotion_item),
    }


def _basket_item_add_kwargs_from_promotion_items(
    promotion_items: list[dict[str, object]],
) -> list[dict[str, object]]:
    return [_basket_item_add_kwargs_from_promotion_item(item) for item in promotion_items]
...
def _basket_item_fingerprints_from_items(items: list[dict[str, object]]) -> list[str]:
    return _present_text_values(item.get("basket_item_fingerprint") for item in items)


def _promotion_item_fingerprints_from_items(items: list[dict[str, object]]) -> list[str]:
    return _present_text_values(item.get("promotion_item_fingerprint") for item in items)


def _basket_promotion_rejection_fingerprints_from_items(items: list[dict[str, object]]) -> list[str]:
    return _present_text_values(item.get("basket_promotion_rejection_fingerprint") for item in items)


def _basket_promotion_rejection_reason_counts_from_items(items: list[dict[str, object]]) -> dict[str, int]:
    counts: dict[str, int] = {}
    for item in items:
        reason = _optional_text(item.get("basket_promotion_rejection_reason"))
...
def _unique_text_values(values: Iterator[object]) -> list[str]:
    normalized: list[str] = []
    seen: set[str] = set()
    for value in values:
        text = _optional_text(value)
...
class RetrievalConstraints:
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.doc_types) > 20:
            raise ValueError("doc_types must not exceed 20 unique filters")
        for dt in self.doc_types:
            if len(dt) > 50:
                raise ValueError("each doc_type filter must not exceed 50 characters")
        if self.date_range is not None:
            if isinstance(self.date_range, (set, dict)):
                raise TypeError("date_range must be an ordered iterable")
            if isinstance(self.date_range, (str, bytes, bytearray)):
                raise TypeError("date_range must be a tuple or list of strings")
            try:
                iter(self.date_range)
            except TypeError as exc:
                raise TypeError("date_range must be an iterable of values or None") from exc
            elements = list(self.date_range)
            if len(elements) != 2:
                raise ValueError("date_range must contain exactly two non-empty values")
            for val in elements:
                if not isinstance(val, str):
                    raise TypeError("each date_range value must be a string")
            normalized = tuple(val.strip() for val in elements)
            if any(not value for value in normalized):
                raise ValueError("date_range must contain exactly two non-empty values")
            for val in normalized:
                if len(val) > 50:
                    raise ValueError("each date_range value must not exceed 50 characters")
        ...
        hint = _optional_text(self.section_hint)
        if hint is not None:
            if len(hint) > 100:
                raise ValueError("section_hint must not exceed 100 characters")
        object.__setattr__(self, "section_hint", hint)
...
class RetrievalQuery:
    ...
    def __post_init__(self) -> None:
        compact_query = _required_compact_text(self.query_text, field_name="query_text")
        if len(compact_query) > 1000:
            raise ValueError("query_text must not exceed 1000 characters")
        object.__setattr__(self, "query_text", compact_query)
        compact_scope = _required_compact_text(self.scope, field_name="scope")
        if len(compact_scope) > 100:
            raise ValueError("scope must not exceed 100 characters")
        object.__setattr__(self, "scope", compact_scope)
        object.__setattr__(
            self,
            "intent",
...
class RetrievalHit:
    ...
    def as_dict(self) -> dict[str, object]:
        ...
        snippet_text_hash = self.provenance.get("snippet_text_hash")
        if isinstance(snippet_text_hash, str) and snippet_text_hash:
            payload["snippet_text_hash"] = snippet_text_hash
...
class RetrievalDocHit:
    ...
    def as_dict(self) -> dict[str, object]:
        ...
        if isinstance(doc_identity_fingerprint, str) and doc_identity_fingerprint:
            payload["doc_identity_fingerprint"] = doc_identity_fingerprint
        doc_source_record = self.provenance.get("doc_source_record")
        if isinstance(doc_source_record, dict):
            payload["doc_source_record"] = copy.deepcopy(doc_source_record)
        doc_source_record_fingerprint = self.provenance.get("doc_source_record_fingerprint")
        if isinstance(doc_source_record_fingerprint, str) and doc_source_record_fingerprint:
            payload["doc_source_record_fingerprint"] = doc_source_record_fingerprint
...
class RetrievalResult:
    ...
    def citation_bundle(self) -> dict[str, object]:
        ...
        basket_promotion_item_fingerprints = _promotion_item_fingerprints_from_items(basket_promotion_items)
        query_date_range = (
        ...
        return {
            ...
            "basket_promotion_item_fingerprints": basket_promotion_item_fingerprints,
    ...
    def retrieval_excerpt_bundle(self) -> dict[str, object]:
        ...
        bundle_context = self._retrieval_bundle_context_snapshot()
        basket_promotion_items = self.basket_promotion_items()
        basket_promotion_rejection_items = self.basket_promotion_rejection_items()
        basket_promotion_rejection_reasons = _unique_text_values(
        ...
        basket_item_fingerprints = _basket_item_fingerprints_from_items(basket_promotion_items)
        basket_promotion_item_fingerprints = _promotion_item_fingerprints_from_items(basket_promotion_items)
        basket_promotion_rejection_fingerprints = _basket_promotion_rejection_fingerprints_from_items(
            basket_promotion_rejection_items
        )
        basket_promotion_rejection_reason_counts = (
            _basket_promotion_rejection_reason_counts_from_items(basket_promotion_rejection_items)
        )
        return {
            **bundle_context,
            "doc_count": len(self.doc_hits),
            ...
            "excerpt_citations": self._excerpt_citation_snapshots(),
            "basket_promotion_items": copy.deepcopy(basket_promotion_items),
            "basket_promotion_rejection_items": copy.deepcopy(basket_promotion_rejection_items),
            ...
            "basket_item_fingerprints": basket_item_fingerprints,
            "basket_promotion_item_fingerprints": basket_promotion_item_fingerprints,
            "basket_promotion_rejection_fingerprints": basket_promotion_rejection_fingerprints,
        }
    ...
    def retrieval_context_bundle(self) -> dict[str, object]:
        ...
        basket_promotion_bundle = downstream_payload["retrieval_basket_promotion_bundle"]
        basket_promotion_provenance = _basket_promotion_provenance_from_snapshot(
            ...
            basket_item_fingerprints=basket_item_fingerprints,
            source_bundle_fingerprint=downstream_payload.get("source_bundle_fingerprint"),
        )
        bundle = {
            "audit_ref": self.audit_ref,
            "result_fingerprint": self.result_fingerprint,
            "fts_match_query_fingerprint": downstream_payload.get("fts_match_query_fingerprint"),
            "query_fingerprint": downstream_payload.get("query_fingerprint"),
            ...
            "retrieval_basket_promotion_bundle": copy.deepcopy(basket_promotion_bundle),
            ...
            "basket_promotion_items": copy.deepcopy(basket_promotion_items),
            "basket_item_add_kwargs": _basket_item_add_kwargs_from_promotion_items(basket_promotion_items),
            "basket_promotion_rejection_items": copy.deepcopy(basket_promotion_rejection_items),
            "basket_promotion_rejection_count": len(basket_promotion_rejection_items),
            "basket_promotion_rejection_reasons": basket_promotion_rejection_reasons,
            "basket_promotion_rejection_reason_counts": basket_promotion_rejection_reason_counts,
    ...
    def basket_promotion_items(self) -> list[dict[str, object]]:
        ...
        doc_provenance_by_id = {
            doc_hit.doc_id: doc_hit.provenance
            for doc_hit in self.doc_hits
        }
        for hit in self.hits:
            if hit.excerpt_id is None:
                continue
            if self._basket_promotion_rejection_reason(hit) is not None:
                continue
            basket_item_id = _basket_item_id_for_excerpt(
                source_strategy=hit.source_strategy,
                excerpt_id=hit.excerpt_id,
            )
            doc_provenance = doc_provenance_by_id.get(hit.doc_id, {})
            doc_source_record = RetrievalService._doc_source_record_for_hit(
                hit,
            ...
            item = {
                ...
                "doc_fingerprint": doc_provenance.get("doc_fingerprint"),
                ...
                "snippet_text_hash": hit.provenance.get("snippet_text_hash"),
                ...
                "basket_promotion_source": _FTS_RESULT_BASKET_PROMOTION_SOURCE,
    ...
    def _hit_has_complete_basket_promotion_provenance(self, hit: RetrievalHit) -> bool:
        return self._basket_promotion_rejection_reason(hit) is None

    def _basket_promotion_rejection_reason(self, hit: RetrievalHit) -> str | None:
        ...
        if (
            ...
            return "missing_basket_item_id"
        score = hit.score
        if not isinstance(score, (int, float)) or isinstance(score, bool) or not math.isfinite(score):
            return "invalid_score"
        source_strategy = provenance.get("source_strategy")
        retrieval_source_strategy = provenance.get("retrieval_source_strategy")
        if (
            source_strategy != _FTS_SOURCE_STRATEGY
            or retrieval_source_strategy != _FTS_SOURCE_STRATEGY
            or hit.source_strategy != _FTS_SOURCE_STRATEGY
        ):
        ...
        if provenance.get("query_fingerprint") != self.diagnostics.get("query_fingerprint"):
            return "stale_query_fingerprint"
        if provenance.get("fts_match_query_fingerprint") != self.diagnostics.get(
            "fts_match_query_fingerprint"
        ):
            return "stale_fts_match_query_fingerprint"
        provenance_result_fingerprint = provenance.get("result_fingerprint")
        if not isinstance(provenance_result_fingerprint, str) or not provenance_result_fingerprint:
            return "missing_result_fingerprint"
        if provenance_result_fingerprint != self.result_fingerprint:
            return "stale_result_fingerprint"
        required_text_field_reasons = {
            "source_hash": "missing_source_hash",
            "doc_identity_fingerprint": "missing_doc_identity_fingerprint",
            "excerpt_fingerprint": "missing_excerpt_fingerprint",
            "excerpt_lookup_fingerprint": "missing_excerpt_lookup_fingerprint",
            "excerpt_text_hash": "missing_excerpt_text_hash",
            "snippet_text_hash": "missing_snippet_text_hash",
        }
        for field, reason in required_text_field_reasons.items():
        ...
        if fts_rank is not None:
            ...
            # ordering and serializes to an invalid JSON token (NaN/Infinity) in
            # the provenance fingerprint, so it can never anchor a stable promotion.
        ...
        # The live match path derives matched_terms from `_query_terms`, which
        # tokenizes a casefolded query with `\w+`. Each term is therefore a single
        # casefolded word token. A non-canonical term (uppercased, punctuated, or
        # multi-token) could never match the live derivation, so it breaks
        # deterministic match provenance. Checking canonical form before the
        # distinctness gate also closes the casefold-collision hole where terms like
        # ["Memo", "memo"] are set-distinct yet collapse under the live dedup.
        if any(
            term != term.casefold() or re.fullmatch(r"\w+", term) is None
        ...
        expected_matched_terms = list(
            RetrievalService._matched_query_terms(
                RetrievalService._query_terms(self.query.query_text),
                hit.excerpt_text,
            )
        )
        if matched_terms != expected_matched_terms:
            return "stale_matched_terms"
        ...
        expected_doc_identity_fingerprint = RetrievalService._stable_fingerprint(
            {
                "doc_id": hit.doc_id,
        ...
        if provenance["doc_identity_fingerprint"] != expected_doc_identity_fingerprint:
            return "stale_doc_identity_fingerprint"
        if doc_provenance.get("doc_identity_fingerprint") != expected_doc_identity_fingerprint:
            return "stale_doc_identity_fingerprint"
        doc_fingerprint = _optional_text(doc_provenance.get("doc_fingerprint"))
        if doc_fingerprint is None:
            return "missing_doc_fingerprint"
        hit_doc_fingerprint = _optional_text(provenance.get("doc_fingerprint"))
        if hit_doc_fingerprint is not None and hit_doc_fingerprint != doc_fingerprint:
            return "stale_doc_fingerprint"
        ...
        if doc_provenance_type != doc_type:
            return "stale_doc_type"
        if doc_source_type != doc_type:
            return "stale_doc_source_type"
        doc_source_strategy = doc_provenance.get(
            "source_strategy",
            doc_provenance.get("retrieval_source_strategy"),
        )
        if doc_source_strategy != _FTS_SOURCE_STRATEGY:
            return "non_fts_doc_source_strategy"
        if doc_provenance.get("retrieval_source_strategy") != _FTS_SOURCE_STRATEGY:
            return "non_fts_doc_source_strategy"
        if (
            doc_provenance.get("retrieval_backend") != "sqlite_fts"
            or doc_provenance.get("retrieval_mode") != "fts_first"
        ):
            return "non_fts_doc_backend"
        expected_doc_source_record = RetrievalService._doc_source_record_for_hit(
            hit,
            doc_provenance=doc_provenance,
        )
        for source_snapshot in (provenance, doc_provenance):
            if not self._matches_expected_doc_source_record(
                source_snapshot,
                expected_doc_source_record=expected_doc_source_record,
            ):
        ...
        if snippet is not None and snippet != hit.excerpt_text:
            return "stale_snippet"
        expected_snippet_hash = hashlib.sha256((snippet or hit.excerpt_text).encode("utf-8")).hexdigest()
        if provenance["snippet_text_hash"] != expected_snippet_hash:
            return "stale_snippet_text_hash"
        hit_span = hit.span
        provenance_span = provenance.get("span")
        if hit_span is None or provenance_span is None:
            return "missing_span"
        canonical_hit_span = RetrievalService._canonicalize_span(hit_span)
        canonical_provenance_span = RetrievalService._canonicalize_span(provenance_span)
        ...
        if provenance["excerpt_lookup_fingerprint"] != expected_lookup_fingerprint:
            return "stale_excerpt_lookup_fingerprint"
        return None

    def basket_promotion_rejection_items(self) -> list[dict[str, object]]:
        """Return deterministic evidence for FTS hits withheld from basket promotion."""
        ...
        for hit in self.hits:
            if hit.excerpt_id is None:
                continue
            reason = self._basket_promotion_rejection_reason(hit)
            if reason is None:
                continue
            doc_provenance = doc_provenance_by_id.get(hit.doc_id)
            expected_doc_source_record = None
            if doc_provenance is not None:
                expected_doc_source_record = RetrievalService._doc_source_record_for_hit(
                    hit,
                    doc_provenance=doc_provenance,
                )
            item = {
                ...
                "doc_identity_fingerprint": hit.provenance.get("doc_identity_fingerprint"),
                "doc_source_record": copy.deepcopy(hit.provenance.get("doc_source_record")),
                "doc_source_record_fingerprint": hit.provenance.get(
                    "doc_source_record_fingerprint"
                ),
                "span": copy.deepcopy(hit.provenance.get("span")),
                "excerpt_text_hash": hit.provenance.get("excerpt_text_hash")
                or hit.provenance.get("hash"),
                "snippet_text_hash": hit.provenance.get("snippet_text_hash"),
                "excerpt_fingerprint": hit.provenance.get("excerpt_fingerprint"),
                "excerpt_lookup_fingerprint": hit.provenance.get("excerpt_lookup_fingerprint"),
                "retrieval_backend": hit.provenance.get("retrieval_backend"),
                ...
                "query_fingerprint": hit.provenance.get("query_fingerprint"),
                "fts_match_query_fingerprint": hit.provenance.get("fts_match_query_fingerprint"),
                "expected_query_fingerprint": self.diagnostics.get("query_fingerprint"),
                "expected_fts_match_query_fingerprint": self.diagnostics.get(
                    "fts_match_query_fingerprint"
                ),
                "result_fingerprint": self.result_fingerprint,
                "canonical_demo_path_steps": list(RETRIEVAL_DEMO_PATH_STEPS),
            }
            if expected_doc_source_record is not None:
                item["expected_doc_source_record"] = copy.deepcopy(expected_doc_source_record)
                item["expected_doc_source_record_fingerprint"] = expected_doc_source_record[
                    "doc_source_record_fingerprint"
                ]
                doc_source_mismatch_fields = self._doc_source_record_mismatch_fields(
                    hit.provenance,
                    expected_doc_source_record=expected_doc_source_record,
                )
                if doc_source_mismatch_fields:
                    item["doc_source_mismatch_fields"] = doc_source_mismatch_fields
                if doc_provenance is not None:
                    doc_hit_source_mismatch_fields = self._doc_source_record_mismatch_fields(
                        doc_provenance,
                        expected_doc_source_record=expected_doc_source_record,
                    )
                    if doc_hit_source_mismatch_fields:
                        item["doc_hit_source_mismatch_fields"] = doc_hit_source_mismatch_fields
            expected_matched_terms = list(
                RetrievalService._matched_query_terms(
                    RetrievalService._query_terms(self.query.query_text),
                    hit.excerpt_text,
            ...
            item["expected_matched_terms"] = expected_matched_terms
            item["expected_match_count"] = len(expected_matched_terms)
            match_mismatch_fields = self._match_mismatch_fields(
                item,
                expected_matched_terms=expected_matched_terms,
            )
            if match_mismatch_fields:
                item["match_mismatch_fields"] = match_mismatch_fields
            query_mismatch_fields = self._query_mismatch_fields(item)
            if query_mismatch_fields:
                item["query_mismatch_fields"] = query_mismatch_fields
            item["basket_promotion_rejection_fingerprint"] = RetrievalService._stable_fingerprint(item)
            items.append(item)
        return items

    @staticmethod
    def _match_mismatch_fields(
        item: dict[str, object],
        *,
        expected_matched_terms: list[str],
    ) -> list[str]:
        mismatch_fields: list[str] = []
        matched_terms = item.get("matched_terms")
        if matched_terms != expected_matched_terms:
            mismatch_fields.append("matched_terms")
        match_count = item.get("match_count")
        if match_count != len(expected_matched_terms):
            mismatch_fields.append("match_count")
        return mismatch_fields

    @staticmethod
    def _query_mismatch_fields(item: dict[str, object]) -> list[str]:
        mismatch_fields: list[str] = []
        query_fingerprint = _optional_text(item.get("query_fingerprint"))
        expected_query_fingerprint = _optional_text(item.get("expected_query_fingerprint"))
        ...
        fts_match_query_fingerprint = _optional_text(item.get("fts_match_query_fingerprint"))
        expected_fts_match_query_fingerprint = _optional_text(
    ...
    @staticmethod
    def _doc_source_record_mismatch_fields(
        source_snapshot: dict[str, object],
        *,
        expected_doc_source_record: dict[str, object],
    ) -> list[str]:
        mismatch_fields: list[str] = []
        embedded_record = source_snapshot.get("doc_source_record")
        if not isinstance(embedded_record, dict):
            mismatch_fields.append("doc_source_record")
        else:
            record_keys = sorted(set(embedded_record) | set(expected_doc_source_record))
            for key in record_keys:
                if embedded_record.get(key) != expected_doc_source_record.get(key):
                    mismatch_fields.append(f"doc_source_record.{key}")
        record_fingerprint = source_snapshot.get("doc_source_record_fingerprint")
        expected_fingerprint = expected_doc_source_record.get("doc_source_record_fingerprint")
        if record_fingerprint != expected_fingerprint:
            mismatch_fields.append("doc_source_record_fingerprint")
        return mismatch_fields

    @staticmethod
    def _matches_expected_doc_source_record(
        source_snapshot: dict[str, object],
        *,
        expected_doc_source_record: dict[str, object],
    ) -> bool:
        ...
        if not isinstance(embedded_record, dict):
            return False
        if embedded_record != expected_doc_source_record:
            return False
        record_fingerprint = source_snapshot.get("doc_source_record_fingerprint")
        expected_fingerprint = expected_doc_source_record.get("doc_source_record_fingerprint")
        return (
            isinstance(record_fingerprint, str)
            and bool(record_fingerprint)
    ...
    def _excerpt_hit_snapshots(self) -> list[dict[str, object]]:
        ...
        rejection_items_by_excerpt_id = {
            str(item["excerpt_id"]): item
            for item in self.basket_promotion_rejection_items()
            if isinstance(item.get("excerpt_id"), str)
        }
        ...
        for hit in self.hits:
            ...
            if hit.excerpt_id is not None:
                ...
                if basket_item is not None and isinstance(basket_item.get("basket_item_fingerprint"), str):
                    ...
                    doc_source_record = basket_item.get("doc_source_record")
                    if isinstance(doc_source_record, dict):
                        snapshot["doc_source_record"] = copy.deepcopy(doc_source_record)
                    doc_source_record_fingerprint = basket_item.get("doc_source_record_fingerprint")
                    if isinstance(doc_source_record_fingerprint, str) and doc_source_record_fingerprint:
                        snapshot["doc_source_record_fingerprint"] = doc_source_record_fingerprint
                    ...
                    snapshot["promotion_item_fingerprint"] = basket_item["promotion_item_fingerprint"]
                    snapshot["basket_promotion_item"] = copy.deepcopy(basket_item)
                    snapshot["basket_promotion_items"] = [copy.deepcopy(basket_item)]
                    ...
                    if isinstance(provenance, dict):
                        if isinstance(basket_item_id, str) and basket_item_id:
                            ...
                            provenance["basket_item_ids"] = [basket_item_id]
                        if isinstance(doc_source_record, dict):
                            provenance["doc_source_record"] = copy.deepcopy(doc_source_record)
                        if isinstance(doc_source_record_fingerprint, str) and doc_source_record_fingerprint:
                            provenance["doc_source_record_fingerprint"] = doc_source_record_fingerprint
                        ...
                        provenance["promotion_item_fingerprint"] = basket_item[
                            "promotion_item_fingerprint"
                        ]
                        provenance["basket_promotion_item"] = copy.deepcopy(basket_item)
                        provenance["basket_promotion_items"] = [copy.deepcopy(basket_item)]
                rejection_item = rejection_items_by_excerpt_id.get(hit.excerpt_id)
                if rejection_item is not None:
                    rejection_reason = rejection_item.get("basket_promotion_rejection_reason")
                    rejection_fingerprint = rejection_item.get("basket_promotion_rejection_fingerprint")
                    if isinstance(rejection_reason, str) and rejection_reason:
                        snapshot["basket_promotion_rejection_reason"] = rejection_reason
                        snapshot["rejection_reason"] = rejection_reason
                    if isinstance(rejection_fingerprint, str) and rejection_fingerprint:
                        snapshot["basket_promotion_rejection_fingerprint"] = rejection_fingerprint
                    snapshot["basket_promotion_rejection_item"] = copy.deepcopy(rejection_item)
                    snapshot["basket_promotion_rejection_items"] = [copy.deepcopy(rejection_item)]
                    provenance = snapshot.get("provenance")
                    if isinstance(provenance, dict):
                        if isinstance(rejection_reason, str) and rejection_reason:
                            provenance["basket_promotion_rejection_reason"] = rejection_reason
                            provenance["rejection_reason"] = rejection_reason
                        if isinstance(rejection_fingerprint, str) and rejection_fingerprint:
                            provenance["basket_promotion_rejection_fingerprint"] = rejection_fingerprint
                        provenance["basket_promotion_rejection_item"] = copy.deepcopy(rejection_item)
                        provenance["basket_promotion_rejection_items"] = [
                            copy.deepcopy(rejection_item)
                        ]
    ...
    def retrieval_basket_promotion_bundle(self) -> dict[str, object]:
        ...
        for hit in self.hits:
            ...
            if basket_item_fingerprint is not None:
                ...
                promotion_item["basket_item_fingerprints"] = [basket_item_fingerprint]
            RetrievalService._with_promotion_item_fingerprint(promotion_item)
            promotion_items.append(promotion_item)
        promotion_item_fingerprints = _promotion_item_fingerprints_from_items(promotion_items)
        basket_promotion_item_fingerprints = _promotion_item_fingerprints_from_items(basket_promotion_items)
        basket_promotion_rejection_fingerprints = _basket_promotion_rejection_fingerprints_from_items(
            basket_promotion_rejection_items
        )
        ...
        promotion_bundle_fingerprint = RetrievalService._stable_fingerprint(
            {
                ...
                "retrieval_evidence_fingerprint": bundle_context["retrieval_evidence_fingerprint"],
                "promotion_item_fingerprints": promotion_item_fingerprints,
                "basket_promotion_item_fingerprints": basket_promotion_item_fingerprints,
                "basket_promotion_rejection_fingerprints": basket_promotion_rejection_fingerprints,
                "basket_promotion_rejection_reasons": basket_promotion_rejection_reasons,
                "basket_promotion_rejection_reason_counts": basket_promotion_rejection_reason_counts,
    ...
    def _retrieval_summary_snapshot(
        ...
        *,
        ...
        doc_source_record_fingerprints = _present_text_values(
            doc_hit.provenance.get("doc_source_record_fingerprint") for doc_hit in self.doc_hits
        )
        ...
        return {
            ...
            "doc_source_record_fingerprints": doc_source_record_fingerprints,
            ...
            "primary_doc_source_record_fingerprint": (
                self.doc_hits[0].provenance.get("doc_source_record_fingerprint")
                if self.doc_hits
                else None
            ),
    ...
    def _retrieval_provenance_snapshot(
        ...
        *,
        ...
        return {
            ...
            "primary_doc_source_record_fingerprint": (
                primary_doc_hit.provenance.get("doc_source_record_fingerprint")
                if primary_doc_hit is not None
                else None
            ),
            "doc_source_record_fingerprints": doc_source_record_fingerprints,
    ...
    def _retrieval_source_bundle_snapshot(
        ...
        *,
        ...
        basket_promotion_rejection_items = copy.deepcopy(
            basket_promotion_bundle_snapshot.get("basket_promotion_rejection_items", [])
        )
        basket_promotion_rejection_reasons = _unique_text_values(
            item.get("basket_promotion_rejection_reason")
            for item in basket_promotion_rejection_items
...
class RetrievalService:
    ...
    def __init__(self, vault_root: Path, *, audit_log: AuditLog, now_fn=None) -> None:
        ...
        self._docindex: DocIndexService | None = None
    ...
    def build_pageindex(self, *, doc_id: str, options: DocIndexBuildOptions | None = None) -> str:
        from exegesis_engine.docindex.service import DocIndexBuildOptions as _DocIndexBuildOptions
        from exegesis_engine.docindex.service import DocIndexService as _DocIndexService

        if self._docindex is None:
            self._docindex = _DocIndexService(self._vault_root, audit_log=self._audit, now_fn=self._now_fn)
        source = self._read_doc_text(doc_id)
        build_opts = options if options is not None else _DocIndexBuildOptions()
    ...
    def retrieve_fts_basket_promotion_items(self, query: RetrievalQuery) -> list[dict[str, object]]:
        """Return canonical FTS promotion items ready for context-basket insertion."""

        return self.retrieve_fts(query).basket_promotion_items()

    def retrieve_fts_basket_item_add_kwargs(self, query: RetrievalQuery) -> list[dict[str, object]]:
        """Return add_basket_item kwargs for the canonical FTS retrieval path."""

        return _basket_item_add_kwargs_from_promotion_items(self.retrieve_fts_basket_promotion_items(query))
    ...
    def retrieve_auto_basket_promotion_items(self, query: RetrievalQuery) -> list[dict[str, object]]:
        """Return canonical FTS promotion items for the FTS-first auto path."""

        return self.retrieve_auto(query).basket_promotion_items()

    def retrieve_auto_basket_item_add_kwargs(self, query: RetrievalQuery) -> list[dict[str, object]]:
        """Return add_basket_item kwargs for the FTS-first auto path."""

        return _basket_item_add_kwargs_from_promotion_items(self.retrieve_auto_basket_promotion_items(query))
    ...
    def _build_doc_hits(
        ...
        *,
        ...
        for doc_id in doc_order:
            ...
            doc_source_record = self._doc_source_record_from_values(
                doc_id=doc_id,
                doc_type=doc_type,
                ...
                source_strategy=primary_strategy_id(),
                retrieval_source_strategy=primary_strategy_id(),
                retrieval_backend=str(retrieval_policy["retrieval_backend"]),
                retrieval_mode=cast(str, retrieval_policy["retrieval_mode"]),
            )
            for hit in doc_hit_list:
                hit.provenance["doc_fingerprint"] = doc_fingerprint
                hit.provenance["doc_source_record"] = copy.deepcopy(doc_source_record)
                hit.provenance["doc_source_record_fingerprint"] = doc_source_record[
                    "doc_source_record_fingerprint"
                ]
            doc_hits.append(
                RetrievalDocHit(
                    ...
                    provenance={
                        ...
                        "doc_fingerprint": doc_fingerprint,
                        "doc_source_record": doc_source_record,
                        "doc_source_record_fingerprint": doc_source_record[
                            "doc_source_record_fingerprint"
                        ],
                        "excerpt_count": len(doc_hit_list),
                        "source_strategy": primary_strategy_id(),
    ...
    def _build_retrieval_manifest(
        ...
        *,
        ...
        doc_source_record_fingerprints = _present_text_values(
            doc_hit.provenance.get("doc_source_record_fingerprint") for doc_hit in doc_hits
        )
        top_excerpt_fingerprints = _present_text_values(
            ...
            [
                {
                    ...
                    "doc_fingerprint": doc_hit.provenance.get("doc_fingerprint"),
                    "doc_identity_fingerprint": doc_hit.provenance.get("doc_identity_fingerprint"),
                    "doc_source_record_fingerprint": doc_hit.provenance.get("doc_source_record_fingerprint"),
                    "doc_rank": doc_hit.provenance.get("doc_rank"),
    ...
    def _build_retrieval_evidence(
        ...
        *,
        ...
        query_constraints_fingerprint = RetrievalService._stable_fingerprint(query_constraints)
        basket_promotion_items = [
            self._with_promotion_item_fingerprint(
                self._with_basket_item_fingerprint(
                    {
                        ...
                        "doc_source_record": self._doc_source_record_from_excerpt_citation(item),
                        "doc_source_record_fingerprint": self._doc_source_record_from_excerpt_citation(item)[
                            "doc_source_record_fingerprint"
                        ],
                        "excerpt_id": item["excerpt_id"],
    ...
    @staticmethod
    def _doc_source_record_from_excerpt_citation(item: dict[str, object]) -> dict[str, object]:
        record = {
        ...
        record["doc_source_record_fingerprint"] = RetrievalService._stable_fingerprint(record)
        return record
    ...
    @staticmethod
    def _doc_source_record_for_hit(
        hit: RetrievalHit,
        *,
        doc_provenance: dict[str, object] | None = None,
    ) -> dict[str, object]:
        """Return the minimal doc identity record required to validate FTS promotion."""

        doc_provenance = doc_provenance or {}
        return RetrievalService._doc_source_record_from_values(
            doc_id=hit.doc_id,
            doc_type=hit.provenance.get("doc_type"),
            source_type=_source_type_from_provenance(hit.provenance),
            title_hint=hit.title_hint,
            source_hash=hit.provenance.get("source_hash"),
            doc_fingerprint=_first_optional_text(
                hit.provenance.get("doc_fingerprint"),
                doc_provenance.get("doc_fingerprint"),
            ),
            doc_identity_fingerprint=hit.provenance.get("doc_identity_fingerprint"),
            source_strategy=hit.source_strategy,
            retrieval_source_strategy=hit.provenance.get(
    ...
    @staticmethod
    def _doc_source_record_from_values(
        *,
        doc_id: object,
        doc_type: object,
        source_type: object,
        title_hint: object,
        source_hash: object,
        doc_fingerprint: object,
        doc_identity_fingerprint: object,
        source_strategy: object,
        retrieval_source_strategy: object,
        ...
        record = {
            ...
            "retrieval_source_strategy": retrieval_source_strategy,
    ...
    def _build_fts_provenance(
        ...
        *,
        ...
        provenance["excerpt_fingerprint"] = self._build_excerpt_fingerprint(
            doc_id=doc_id,
            excerpt_id=excerpt_id,
            span=cast(dict[str, object], provenance["span"]),
            text_hash=text_hash,
    ...
    def _normalize_excerpt_payload(
        ...
        *,
        ...
        has_explicit_provenance = isinstance(excerpt.get("provenance"), dict)
        ...
        raw_source_hash = normalized.get("source_hash")
        provenance_source_hash = provenance.get("source_hash")
        has_explicit_source_hash = isinstance(raw_source_hash, str) and bool(raw_source_hash)
        has_explicit_provenance_source_hash = isinstance(provenance_source_hash, str) and bool(
        ...
        elif not isinstance(doc_type, str) or not doc_type:
        ...
        doc_fingerprint = _first_optional_text(
            normalized.get("doc_fingerprint"),
            provenance.get("doc_fingerprint"),
            doc_meta.get("doc_fingerprint"),
        )
        expected_doc_fingerprint = None
        if (
            doc_id_value is not None
            and isinstance(source_hash, str)
            and source_hash
            ...
            expected_doc_fingerprint = RetrievalService._build_lookup_doc_fingerprint(
                doc_id=doc_id_value,
                source_hash=source_hash,
                doc_type=doc_type,
            )
        if doc_fingerprint is not None and expected_doc_fingerprint is not None:
            if doc_fingerprint != expected_doc_fingerprint:
                doc_fingerprint = None
                normalized.pop("doc_fingerprint", None)
        elif doc_fingerprint is None and expected_doc_fingerprint is not None:
            doc_fingerprint = expected_doc_fingerprint
        if doc_fingerprint is not None:
            normalized["doc_fingerprint"] = doc_fingerprint

        source_type = _first_optional_text(normalized.get("source_type"), provenance.get("source_type"))
        if source_type is None and not has_explicit_provenance and isinstance(doc_type, str) and doc_type:
            source_type = doc_type
        if source_type is not None:
        ...
        if snippet is not None:
            ...
            normalized["snippet_text_hash"] = hashlib.sha256(snippet.encode("utf-8")).hexdigest()
        ...
        if basket_promotion_item is not None:
            retrieval_basket_promotion_bundle = self._excerpt_lookup_basket_promotion_bundle(
                basket_promotion_item
            )
            ...
            normalized["doc_source_record"] = copy.deepcopy(basket_promotion_item["doc_source_record"])
            normalized["doc_source_record_fingerprint"] = basket_promotion_item[
                "doc_source_record_fingerprint"
            ]
            ...
            normalized["retrieval_basket_promotion_bundle"] = retrieval_basket_promotion_bundle
            normalized["basket_item_add_kwargs"] = copy.deepcopy(
                retrieval_basket_promotion_bundle["basket_item_add_kwargs"]
            )
        else:
            for key in (
                "basket_promotion_item",
                "basket_promotion_items",
                "basket_promotion_source",
                "basket_item_id",
                "basket_item_ids",
                "basket_item_fingerprint",
                "basket_item_fingerprints",
                ...
                "doc_source_record_fingerprint",
                "retrieval_basket_promotion_bundle",
                "basket_item_add_kwargs",
            ):
                normalized.pop(key, None)
            normalized["basket_promotion_count"] = 0
            normalized["basket_promotion_ready"] = False
        ...
        if snippet is not None:
            ...
            normalized_provenance["snippet_text_hash"] = normalized["snippet_text_hash"]
        if canonical_span is not None:
        ...
        normalized_provenance["canonical_demo_path_steps"] = list(RETRIEVAL_DEMO_PATH_STEPS)
        ...
        if isinstance(normalized.get("doc_source_record"), dict):
            normalized_provenance["doc_source_record"] = copy.deepcopy(normalized["doc_source_record"])
        if isinstance(normalized.get("doc_source_record_fingerprint"), str):
            normalized_provenance["doc_source_record_fingerprint"] = normalized[
                "doc_source_record_fingerprint"
            ]
        ...
        if isinstance(normalized.get("retrieval_basket_promotion_bundle"), dict):
            normalized_provenance["retrieval_basket_promotion_bundle"] = copy.deepcopy(
                normalized["retrieval_basket_promotion_bundle"]
            )
        normalized["provenance"] = normalized_provenance
        return normalized

    @staticmethod
    def _excerpt_lookup_basket_promotion_bundle(item: dict[str, object]) -> dict[str, object]:
        """Return the single-excerpt lookup bundle consumed by context-basket gather flows."""

        promotion_item = copy.deepcopy(item)
        basket_item_add_kwargs = [_basket_item_add_kwargs_from_promotion_item(promotion_item)]
        promotion_bundle_fingerprint = RetrievalService._stable_fingerprint(
            {
                "promotion_target": "context_basket",
                "basket_promotion_source": item.get("basket_promotion_source"),
                "excerpt_lookup_fingerprint": item.get("excerpt_lookup_fingerprint"),
                "promotion_item_fingerprints": [item.get("promotion_item_fingerprint")],
                "basket_promotion_item_fingerprints": [item.get("promotion_item_fingerprint")],
            }
        )
        return {
            "promotion_target": "context_basket",
            ...
            "retrieval_backend": item.get("retrieval_backend"),
            "retrieval_mode": item.get("retrieval_mode"),
            "retrieval_policy": copy.deepcopy(item.get("retrieval_policy")),
            "lookup_resolution": item.get("lookup_resolution"),
            "excerpt_lookup_fingerprint": item.get("excerpt_lookup_fingerprint"),
            "promotion_item_count": 1,
            "promotion_items": [promotion_item],
            "basket_promotion_items": [copy.deepcopy(item)],
            "basket_promotion_count": 1,
            "basket_promotion_ready": True,
            "basket_item_ids": [item["basket_item_id"]],
            "basket_item_fingerprints": [item["basket_item_fingerprint"]],
            "basket_item_add_kwargs": basket_item_add_kwargs,
            "promotion_bundle_fingerprint": promotion_bundle_fingerprint,
        }
    ...
    def _excerpt_lookup_basket_promotion_item(
        ...
        *,
        ...
        if not isinstance(excerpt_id, str) or not excerpt_id:
            return None
        source_type = _optional_text(excerpt.get("source_type"))
        if (
            doc_id is None
            or doc_type is None
            or source_type is None
            or source_hash is None
            or doc_fingerprint is None
            or span is None
            ...
            or source_type != doc_type
        ):
            return None
        excerpt_text = excerpt.get("excerpt_text")
        snippet = _first_optional_text(excerpt.get("snippet"), excerpt_text)
        snippet_text_hash = excerpt.get("snippet_text_hash")
        if (
            not isinstance(excerpt_text, str)
            or not excerpt_text
            or snippet != excerpt_text
            or snippet_text_hash != text_hash
        ):
            return None
        basket_item_id = _basket_item_id_for_excerpt(
            source_strategy=source_strategy,
        ...
        if excerpt.get("excerpt_fingerprint") != expected_excerpt_fingerprint:
            return None
        lookup_query_fingerprint = RetrievalService._stable_fingerprint(
            {
                "excerpt_id": excerpt_id,
                "source_strategy": source_strategy,
                "retrieval_backend": retrieval_backend,
        ...
        lookup_result_fingerprint = RetrievalService._stable_fingerprint(
            {
                "doc_id": doc_id,
                "excerpt_id": excerpt_id,
                "source_hash": source_hash,
                "excerpt_fingerprint": excerpt.get("excerpt_fingerprint"),
                "excerpt_lookup_fingerprint": excerpt_lookup_fingerprint,
                "query_fingerprint": lookup_query_fingerprint,
            }
        )
        item = {
            "item_id": basket_item_id,
            ...
            "retrieval_policy": copy.deepcopy(retrieval_policy),
            "lookup_resolution": lookup_resolution,
            "basket_promotion_source": _FTS_EXCERPT_LOOKUP_BASKET_PROMOTION_SOURCE,
            "result_fingerprint": lookup_result_fingerprint,
            "query_fingerprint": lookup_query_fingerprint,
            "fts_match_query_fingerprint": lookup_query_fingerprint,
        ...
        doc_source_record = RetrievalService._doc_source_record_from_excerpt_citation(item)
        item["doc_source_record"] = doc_source_record
        item["doc_source_record_fingerprint"] = doc_source_record["doc_source_record_fingerprint"]
