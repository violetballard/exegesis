...
def _validate_cli_entrypoints() -> None:
    ...
    for entrypoint in _CLI_ENTRYPOINTS:
        ...
        target = _CLI_ENTRYPOINT_TARGETS.get(normalized_entrypoint, normalized_entrypoint)
        if command_spec_for(COMMAND_SPECS, target) is None:
            raise ValueError(f"Unknown CLI command entrypoint: {entrypoint}")

    declared_entrypoints = set(seen_entrypoints)
    for entrypoint, target in _CLI_ENTRYPOINT_TARGETS.items():
        normalized_entrypoint = _normalize_token(entrypoint)
        normalized_target = _normalize_token(target)
        if normalized_entrypoint not in declared_entrypoints:
            raise ValueError(f"Unknown CLI entrypoint target mapping: {entrypoint}")
        if not normalized_target:
            raise ValueError(f"Empty CLI entrypoint target for: {entrypoint}")
        if command_spec_for(COMMAND_SPECS, normalized_target) is None:
            raise ValueError(f"Unknown CLI entrypoint target: {entrypoint} -> {target}")
...
COMMAND_SPECS: tuple[CommandSpec, ...] = (
    ...
    CommandSpec(
        name="notebook",
        description="Manage notebook context compaction and budget.",
        flow_step="notebook-compaction",
    ),
)
...
_CLI_ENTRYPOINT_TARGETS: dict[str, str] = {
    "patch-apply": "diff-preview",
    "patch-reject": "diff-preview",
...
DEMO_PATH_STEPS_BY_FLOW_STEP: tuple[tuple[str, str], ...] = (
    ...
    ("notebook-compaction", "notebook-compaction"),
)
...
def command_cli_lookup_table() -> tuple[tuple[str, str], ...]:
    ...
    for entrypoint in _CLI_ENTRYPOINTS:
        target = _CLI_ENTRYPOINT_TARGETS.get(_normalize_token(entrypoint), entrypoint)
        spec = command_spec_for(COMMAND_SPECS, target)
        if spec is None:
            raise ValueError(f"Unknown CLI command entrypoint: {entrypoint}")
...
def command_mvp_demo_path_next_blocker_line(program: str = "qual-bootstrap") -> str:
    return command_demo_path_next_blocker_line(program=program, flow_steps=command_mvp_flow_steps())


def command_mvp_demo_path_blocker_lines(program: str = "qual-bootstrap") -> tuple[str, ...]:
    return command_demo_path_blocker_lines(program=program, flow_steps=command_mvp_flow_steps())
