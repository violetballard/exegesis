...
def build_prompt(lane: str, workdir: str) -> str:
    ...
    return (
        ...
        f"{engine_priority}\n\n"
        "Before making changes, consult these documents from the worktree/repo root without dumping them fully into context.\n"
        "Use `rg` and narrow `sed -n` ranges for the sections relevant to this lane; do not `cat` every required doc end-to-end in local mode:\n"
        "- AGENTS.md\n"
        "- INTEGRATION.md\n"
        "- THREAD_OWNERSHIP.md\n"
        "- ROADMAP.md\n"
        ...
        "- For files longer than roughly 20 KB or 250 lines, use `rg`, `nl -ba | sed -n '<start>,<end>p'`, or tool reads with explicit small limits.\n"
        "- Keep source reads to the smallest relevant section, normally <=120 lines at a time.\n"
