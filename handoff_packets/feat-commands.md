...
- Canonical demo-path step advanced: step 1 of the canonical demo path, `open project/document` via the CLI, with this slice keeping that operator-facing entry surface stable so the downstream `retrieve relevant material` and `preview and apply or reject a patch` steps remain executable while Textual remains disabled.
...
- Deterministic validation sentence: deterministic CLI contract validation directly strengthens step 1, `open project/document`, because it fails fast before the operator enters the rest of the MVP loop through a parser surface that no longer matches the canonical catalog while Textual remains disabled.
...
- Step-1 strengthening sentence: deterministic CLI contract validation makes the `open project/document` step more real because the exact parser-backed entrypoints the operator uses are now forced to match the canonical catalog before the CLI-first MVP loop can start while Textual remains disabled.
...
- `ROADMAP.md` Milestone 3 exit criterion `CLI can still execute the MVP loop while Textual remains disabled` because deterministic parser/catalog alignment hardens the exact `open project/document` entry step that gates the rest of the CLI-first loop.
