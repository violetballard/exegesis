...
- Canonical demo-path step advanced: CLI compatibility for the existing `open project/document` entry step, so the CLI can still execute the MVP loop while Textual remains disabled.
