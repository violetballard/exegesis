...
- Canonical demo-path mapping sentence: this slice makes the `patch-review` step more real because `command_cli_contract()` in `src/qual/commands/catalog.py` now returns the validated canonical parser-entrypoint projection for the live `diff-preview` and `diff` patch-review surface after `project-open`/document open and retrieval, and the regression coverage in `tests/unit/test_commands_catalog.py` proves both the canonical catalog order and the fail-fast parser-drift rejection that keeps this operator contract deterministic while Textual remains disabled.
...
- `ROADMAP.md` canonical demo path because the concrete operator step protected is `patch-review` in `project-open -> retrieval -> patch-review -> apply/reject -> persist -> export-handoff`, where `command_cli_contract()` keeps the `diff-preview` and `diff` entrypoints tied to the canonical catalog and the regression coverage in `tests/unit/test_commands_catalog.py` proves they fail fast on drift.
