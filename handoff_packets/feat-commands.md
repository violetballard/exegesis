...
- reviewer-fix satisfaction note:
  - this reissued handoff now states the exact canonical demo-path step it advances and narrows the vision mapping to the canonical engine contract within `PRODUCT_VISION.md` capability 4, not the broader operator-surface expansion; it does not claim `Auditable state and workflow`
- reviewer-required canonical demo-path handoff field: this handoff explicitly advances the current CLI-first operator path `project-open -> retrieval -> patch-review` by proving the active CLI fallback rejects parser/catalog drift before an operator reaches the `preview`, `apply`, or `reject` entrypoints in the current engine-first MVP loop
- canonical demo-path step advanced: the current CLI-first operator path through `project-open -> retrieval -> patch-review`, specifically the `preview and apply or reject a patch` step on the active MVP engine-first path (`Engine stability`, `FTS-first retrieval`, `A2UI contracts with CLI fallback`)
- Milestone 3 exit-criterion mapping: this is a concrete Milestone 3 CLI-compatibility safeguard for the active demo path because it keeps the current CLI-first operator sequence `project-open -> retrieval -> patch-review` executable while Textual remains disabled by proving the active `preview`, `apply`, and `reject` `parse_args()` surface is deterministic and drift-checked from `src/main.py::_dispatch()`
...
- vision capability affected:
  - `PRODUCT_VISION.md` capability 4 `Operator-first control surface`, specifically its canonical engine contract requirement (`Engine emits structured outputs that can be consumed by CLI now and Exegesis Console next`, `engine contracts come first`, and `UI can evolve on top without changing core contracts`): this slice is limited to keeping the existing `preview`, `apply`, and `reject` CLI contract deterministic and drift-checked from the real operator path, without claiming broader operator-surface expansion or auditable-state/workflow progress
