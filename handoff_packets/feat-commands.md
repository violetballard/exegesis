...
- Approval basis pin:
  - Re-review remains pinned to implementation commit `b551c0786f650ef38bc72f97841f4bdbb2de1f6e` and the two implementation files only: `src/qual/commands/catalog.py` and `tests/unit/test_commands_catalog.py`.
...
- Canonical demo-path mapping sentence: this slice specifically strengthens `continue working without losing context` because a CLI fallback operator now fails immediately if the supporting `project-open` / `retrieval` / `patch-review` parser surface drops `diff` or any other declared entrypoint while the canonical command sequence still appears unchanged.
...
- Re-review refresh note: packet metadata refreshed on `2026-04-24` to pin re-review to the single implementation commit above, then re-verified after rerunning the full required local gates at `2026-04-24T11:45:09Z UTC`.
...
3. `continue working without losing context`: updated [handoff_packets/feat-commands.md](/Users/doctor-violet/.codex/worktrees/5494/qual/handoff_packets/feat-commands.md:1), [THREAD_PACKET.md](/Users/doctor-violet/.codex/worktrees/5494/qual/THREAD_PACKET.md:1), and [THREAD.md](/Users/doctor-violet/.codex/worktrees/5494/qual/THREAD.md:1) so the re-review packet points to commit `b551c0786f650ef38bc72f97841f4bdbb2de1f6e`, names the exact primary canonical demo-path step explicitly, and ties each completed task to that step.
...
- `make ci` -> passed
- Verification rerun timestamp: `2026-04-24T11:45:09Z UTC`
