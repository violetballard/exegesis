...
- Reviewer-fix closure note: this refresh explicitly satisfies the reviewer-requested missing handoff field by naming `open project/document` as the primary canonical demo-path step advanced, naming the dependent CLI loop surfaces `retrieval`, `patch-review`, and `export-handoff`, tying deterministic CLI contract validation to the active Milestone 3 CLI-first loop, and preserving the resubmission as packet-only alignment rather than a new implementation slice.
- High-risk framing note: this remains a high-risk shared-file handoff because it touched one public command-contract file in `src/qual/commands/catalog.py` plus one explicitly approved shared regression file in `tests/unit/test_commands_catalog.py`; the risk reason is that parser-surface drift at the public CLI contract can silently break the engine-first MVP loop if the contract is allowed to diverge.
...
- Reviewer-required step mapping: the primary canonical demo-path step advanced is `open project/document`, and the same deterministic CLI contract hardening directly protects the existing `retrieval`, `patch-review`, and `export-handoff` command entrypoints that the Milestone 3 CLI-backed MVP loop still depends on while Textual remains disabled.
...
- Risk reason: this is high-risk shared-file work because it changes a public CLI contract in `src/qual/commands/catalog.py` and relies on one approved shared regression file, so parser-surface drift here can undermine the deterministic engine-first MVP loop at its command entrypoints.
