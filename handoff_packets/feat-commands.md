...
- reviewer-required canonical demo-path handoff field: this handoff explicitly advances the canonical `preview and apply or reject a patch` step by making the active CLI fallback reject parser/catalog drift before an operator reaches the `preview`, `apply`, or `reject` entrypoints in the current engine-first MVP loop
