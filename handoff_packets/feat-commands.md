...
- Canonical demo-path step advanced: Milestone 3 CLI-compatibility hardening for the existing operator surface that covers `open project/document`, `retrieve relevant material`, `preview and apply or reject a patch`, and `continue working`.
- Explicit handoff sentence: This work makes that existing CLI operator surface more real by keeping the parser-facing command contract deterministic for the current smoke-route entrypoints, so token drift fails before an operator reaches `open`, `retrieve`, `patch-review`/`apply-patch`/`reject-patch`, or follow-on `continue` commands while Textual remains disabled.
- MVP focus tie-in: this is Milestone 3 CLI-compatibility hardening for the existing command surface in support of the current MVP emphasis on `A2UI` contracts with CLI fallback, not new workflow capability or surface-area expansion.
- Concrete blocker removed: without this guard, parser/catalog drift could silently reorder, add, or drop the operator-facing CLI tokens that must stay stable across the existing `open`, `retrieve`, `patch-review`/`apply-patch`/`reject-patch`, and `continue` smoke-route entrypoints, so the current MVP loop could present a stale smoke-check contract while the real CLI surface no longer matches the catalog.
...
- Roadmap alignment: `ROADMAP.md` Milestone 3 exit criterion `Contract changes documented and intentional` plus `AGENTS.md` active MVP note `A2UI contracts with CLI fallback`; this handoff is a narrow CLI-compatibility hardening change that documents and locks the deterministic contract for the existing `open`, `retrieve`, `patch-review`/`apply-patch`/`reject-patch`, and `continue` command surface while Textual remains disabled and without claiming new workflow coverage.
...
- plan complete: the handoff is narrowed to the reviewed `command_cli_contract()` slice and explicitly mapped to Milestone 3 CLI-compatibility hardening for the existing `open`/`retrieve`/`patch-review`/`continue` command surface
...
- scope completed:
  ...
  - refreshed the handoff packet so the review claim, Milestone 3 CLI-compatibility mapping, and file list match the reviewed command-catalog slice exactly
...
- roadmap item(s) affected:
  - `ROADMAP.md` Milestone 3 exit criterion `Contract changes documented and intentional`: preserve the deterministic CLI contract for the existing operator surface by failing fast when parser/catalog drift mutates the `open`, `retrieve`, `patch-review`/`apply-patch`/`reject-patch`, or `continue` command surface
