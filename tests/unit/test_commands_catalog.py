...
class CommandCatalogTests(unittest.TestCase):
    ...
    def test_command_cli_contract_matches_the_catalog_order(self) -> None:
        ...
        self.assertEqual(
            live_parser_surface,
            tuple(
                (name, tuple(token for token, resolved_name in contract.lookup_table if resolved_name == name))
                for name in contract.canonical_names
            ),
        )
    ...
    def test_command_cli_contract_rejects_live_cli_entrypoint_constant_drift_with_stable_canonical_names(
        self,
    ) -> None:
        baseline_contract = command_catalog.command_cli_contract()
        parser_surface_with_mutated_diff_alias = (
    ...
    def test_command_cli_contract_rejects_argparse_subparser_drift_with_stable_canonical_names(
        self,
    ) -> None:
        baseline_contract = command_catalog.command_cli_contract()
        build_parser = qual_cli._build_parser

        def build_drifted_parser() -> object:
            parser = build_parser()
            p_diff_alias = qual_cli._subparser_action(parser).add_parser(
                "review-patch",
                help="Unexpected diff-preview parser alias",
            )
        ...
        command_catalog.command_cli_contract.cache_clear()
        with patch.object(qual_cli, "_build_parser", side_effect=build_drifted_parser):
            parser_surface = qual_cli.parser_cli_entrypoints()
            parser_tokens = tuple(
                token
                for _, entrypoints in parser_surface
                for token in entrypoints
            )
            self.assertEqual(tuple(name for name, _ in parser_surface), baseline_contract.canonical_names)
            self.assertIn("review-patch", parser_tokens)
            self.assertNotEqual(
                parser_surface,
                command_catalog._declared_cli_entrypoint_projection(command_specs()),
            )
    ...
    def test_parse_args_rejects_live_cli_entrypoint_constant_drift_with_stable_canonical_names(
        self,
    ) -> None:
        baseline_contract = command_catalog.command_cli_contract()
    ...
    def test_command_cli_contract_validation_rejects_alias_drift_with_stable_canonical_names(self) -> None:
        ...
        drifted_contract = replace(
            contract,
            tokens=("bootstrap", "diff-preview", "context-basket", "terminal"),
            lookup_table=(
                ("bootstrap", "bootstrap"),
                ("diff-preview", "diff-preview"),
                ("context-basket", "context-basket"),
        ...
        self.assertEqual(drifted_contract.canonical_names, contract.canonical_names)
        self.assertNotEqual(
            command_catalog._cli_entrypoint_projection_from_contract(drifted_contract),
            command_catalog._authoritative_cli_entrypoint_projection(command_specs()),
        )
        with self.assertRaisesRegex(ValueError, "Command CLI catalog entrypoint projection is inconsistent"):
    ...
    def test_command_cli_shim_helpers_rewrite_legacy_surface_tokens_to_primary_argv(self) -> None:
        ...
        self.assertEqual(command_cli_shim_primary_token("review"), "diff-preview")
        ...
        self.assertEqual(command_cli_shim_primary_token("apply"), "terminal")
        self.assertEqual(command_cli_shim_primary_token("save"), "terminal")
        self.assertEqual(command_cli_shim_primary_token("handoff"), "terminal")
        ...
        self.assertEqual(
            command_cli_shim_argv(("apply",)),
            ("terminal", "--operation-kind", "terminal_tool_orchestration", "--message", "Apply patch"),
        )
        self.assertEqual(
            command_cli_shim_argv(("save",)),
            ("terminal", "--operation-kind", "terminal_synthesis_request", "--message", "Persist and continue"),
        )
        self.assertEqual(
            command_cli_shim_argv(("handoff",)),
            ("terminal", "--operation-kind", "terminal_synthesis_request", "--message", "Export handoff"),
        )
        self.assertEqual(
    ...
    def test_command_cli_entry_argv_normalizes_parser_ready_invocations(self) -> None:
        ...
        self.assertEqual(
            command_cli_entry_argv(["handoff"]),
            ("terminal", "--operation-kind", "terminal_synthesis_request", "--message", "Export handoff"),
        )
        self.assertEqual(
    ...
    def test_command_resolve_argv_rewrites_to_primary_cli_tokens(self) -> None:
        ...
        review = command_resolve_argv(("review",))
        self.assertTrue(review.matched)
        self.assertEqual(review.canonical_name, "diff-preview")
        self.assertEqual(review.argv, ("diff-preview", "--original", "before", "--proposed", "after"))
        self.assertEqual(review.normalized_token, "review")

        apply = command_resolve_argv(("apply",))
        self.assertTrue(apply.matched)
        self.assertEqual(apply.canonical_name, "terminal")
        self.assertEqual(
            apply.argv,
            ("terminal", "--operation-kind", "terminal_tool_orchestration", "--message", "Apply patch"),
        )
    ...
    def test_command_cli_contract_rejects_alias_substitution_in_parser_surface_when_canonical_order_still_matches(
        ...
        with patch.object(
            ...
            self.assertEqual(command_catalog.command_spec_for(command_specs(), "open").name, "bootstrap")
            self.assertEqual(
    ...
    def test_command_cli_contract_rejects_missing_canonical_token_when_alias_still_resolves(self) -> None:
        command_catalog.command_cli_contract.cache_clear()
        with patch.object(
            command_catalog,
            "_live_parser_cli_entrypoints",
    ...
    def test_command_cli_contract_rejects_context_alias_drift_with_stable_canonical_names(
        self,
    ) -> None:
        command_catalog.command_cli_contract.cache_clear()
        with patch.object(
    ...
    def test_command_demo_trusted_surface_contract_exposes_self_describing_demo_tokens(self) -> None:
        ...
        self.assertEqual(apply_patch.flow_step, "apply-patch")
        self.assertEqual(
            apply_patch.argv,
            ("terminal", "--operation-kind", "terminal_tool_orchestration", "--message", "Apply patch"),
        )
    ...
    def test_command_demo_workflow_contract_bundles_the_trusted_mvp_loop(self) -> None:
        ...
        self.assertEqual(
            ...
            (
                ...
                ("apply-patch", "terminal", "apply-patch"),
                ("reject-patch", "terminal", "reject-patch"),
    ...
    def test_command_demo_next_action_contract_bundles_trusted_follow_up_actions(self) -> None:
        contract = command_demo_next_action_contract("review")
        self.assertEqual(contract, command_mvp_next_action_contract("review"))
        self.assertEqual(contract.entries, command_demo_next_action_catalog("patch-review"))
        self.assertEqual(contract.lookup_table, command_demo_next_action_lookup_table("patch-review"))
        self.assertEqual(
            contract.invocation_table,
        ...
        self.assertEqual(
            tuple((entry.target_token, entry.canonical_name, entry.flow_step) for entry in contract.entries),
            (
                ("apply-patch", "terminal", "apply-patch"),
                ("reject-patch", "terminal", "reject-patch"),
            ),
    ...
    def test_command_demo_next_action_contract_tracks_custom_specs(self) -> None:
        specs = (
            ...
            CommandSpec(
                name="terminal",
                aliases=("persist-continue", "apply-patch", "reject-patch", "export-handoff"),
                cli_tokens=("terminal",),
                smoke_argv=(
                    "terminal",
                    "--operation-kind",
                ...
                preferred_surface_tokens=("apply-patch", "persist-continue", "export-handoff"),
                flow_step="export-handoff",
            ),
        )

        contract = command_demo_next_action_contract("review", specs)
        self.assertEqual(contract.source_token, "patch-review")
        self.assertEqual(
            tuple((entry.target_token, entry.argv) for entry in contract.entries),
            (
                (
                    "apply-patch",
                    ("terminal", "--operation-kind", "terminal_tool_orchestration", "--message", "Apply patch"),
    ...
    def test_command_cli_entry_argv_normalizes_demo_compatibility_tokens(self) -> None:
        demo_flow_steps = command_mvp_flow_steps()

        self.assertEqual(
            command_cli_entry_argv_for(command_specs(), ("review",), demo_flow_steps),
            ("diff-preview",),
        )
        self.assertEqual(
            command_cli_entry_argv_for(command_specs(), ("apply",), demo_flow_steps),
            ("terminal", "--operation-kind", "terminal_tool_orchestration", "--message", "Apply patch"),
        )
        self.assertEqual(
    ...
    def test_command_cli_entry_argv_normalizes_demo_compatibility_tokens_for_custom_specs(self) -> None:
        ...
        self.assertEqual(
            command_cli_entry_argv_for(specs, ("review",), command_mvp_flow_steps()),
            ("review-diff", "--original", "before", "--proposed", "after"),
        )
        self.assertEqual(
    ...
    def test_command_resolve_argv_for_demo_compatibility_tokens_uses_parser_ready_fallbacks_for_custom_specs(self) -> None:
        ...
        self.assertEqual(resolved.flow_step, "apply-patch")
        self.assertEqual(
            resolved.argv,
            ("terminal", "--operation-kind", "terminal_tool_orchestration", "--message", "Apply patch"),
        )
    ...
    def test_public_workflow_surface_aliases_track_the_current_mvp_trusted_surface(self) -> None:
        self.assertEqual(command_workflow_surface_contract(), command_mvp_trusted_surface_contract())
        self.assertEqual(command_workflow_surface_tokens(), command_mvp_trusted_surface_tokens())
        self.assertEqual(
            command_workflow_surface_flow_lookup_table(),
            command_mvp_trusted_surface_flow_lookup_table(),
        )
