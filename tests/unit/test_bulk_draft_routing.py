...
class BulkDraftRoutingTests(unittest.TestCase):
    def test_128gb_pack_supports_best_ondemand_not_resident(self) -> None:
        pack = PackMetadata(memory_tier_gb=128, installed_models=(BEST_MODEL_ID,))
        runtime = _RuntimeStub({BEST_MODEL_ID})
        caps = compute_capabilities(pack, runtime)
    ...
    def test_256gb_pack_supports_best_resident(self) -> None:
        pack = PackMetadata(memory_tier_gb=256, installed_models=(BEST_MODEL_ID,))
        runtime = _RuntimeStub({BEST_MODEL_ID})
        caps = compute_capabilities(pack, runtime)
        self.assertTrue(caps.supports_best_bulk_resident)

    def test_discussion_and_conclusion_select_best_when_supported(self) -> None:
    ...
    def test_thresholds_for_methods_findings_and_general(self) -> None:
        caps = BulkDraftCapabilities(True, False, True)
        methods_fast = route_bulk_draft(
            _payload(section_type="methods", target_word_count=2499, capabilities=caps)
        )
        methods_best = route_bulk_draft(
            _payload(section_type="methods", target_word_count=2500, capabilities=caps)
        )
        intro_fast = route_bulk_draft(
            _payload(section_type="introduction", target_word_count=1499, capabilities=caps)
        )
        intro_best = route_bulk_draft(
            _payload(section_type="introduction", target_word_count=1500, capabilities=caps)
        )
        self.assertEqual(methods_fast.bulk_draft_tier, "fast")
        self.assertEqual(methods_best.bulk_draft_tier, "best")
        self.assertEqual(methods_best.bulk_draft_reason, "word_count_threshold_methods_findings")
        self.assertEqual(intro_fast.bulk_draft_tier, "fast")
        self.assertEqual(intro_best.bulk_draft_tier, "best")
        self.assertEqual(intro_best.bulk_draft_reason, "word_count_threshold_general")
    ...
    def test_best_path_on_128gb_uses_drafting_mode_load_cycle(self) -> None:
    ...
    def test_bulk_requires_outline_id(self) -> None:
    ...
    def test_editor_pass_always_runs_after_fast(self) -> None:
        editor_calls: list[str] = []
        ...
        self.assertEqual(result.patch_proposal, "fast draft edited")
...
def _payload(
    *,
    ...
    from src.qual.engine.bulk_draft import BulkDraftRoutingInput
