...
3. Do not imply unrelated retrieval tooling scope outside the lane-owned retrieval packages.
4. Treat PageIndex and embeddings mentions as deferred markers only, not active MVP paths.
5. Make the roadmap and vision fields read as handoff-boundary documentation, not feature delivery.
...
6. Document the `section:` compatibility boundary as an intentional MVP limitation until fallback support is restored.
...
- Ownership note: this fix changes only lane-owned docs artifacts, does not promote any shared or non-owned source files, and explicitly excludes `src/qual/engine/tools/retrieval_tools.py` from the handoff.
