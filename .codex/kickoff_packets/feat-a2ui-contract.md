...
- Vision capability affected: `PRODUCT_VISION.md` Capability 5, Agent-to-UI protocol (`A2UI`), specifically the capability that agent-emitted structured presentation artifacts (cards, sections, actions, metadata) remain consumable by CLI first, then `Exegesis Console`, then future Studio UI; deterministic action ordering keeps the CLI fallback stable.
