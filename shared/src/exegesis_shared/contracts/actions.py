...
_ACTION_TO_CONTROL: dict[str, str] = {
...
class ActionRef:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.id, str) or not self.id.strip():
            raise ValueError("ActionRef.id must be a non-empty string")
        if self.id != self.id.strip():
            raise ValueError("ActionRef.id must be normalized")
        if not isinstance(self.label, str) or not self.label.strip():
            raise ValueError("ActionRef.label must be a non-empty string")
        if not isinstance(self.payload, dict):
            raise ValueError("ActionRef.payload must be a dict")
        for key in self.payload:
            if not isinstance(key, str) or not key.strip():
                raise ValueError("Payload field names must be non-empty strings")
            if key != key.strip():
                raise ValueError("ActionRef.payload keys must be normalized")
        if self.confirm is not None:
            if not isinstance(self.confirm, dict):
                raise ValueError("ActionRef.confirm must be a dict or None")
            if "title" not in self.confirm:
                raise ValueError("ActionRef.confirm must contain key 'title'")
            for k, v in self.confirm.items():
                if not isinstance(k, str) or not k.strip():
                    raise ValueError("ActionRef.confirm keys must be non-empty strings")
                if k != k.strip():
                    raise ValueError("ActionRef.confirm keys must be normalized")
                if not isinstance(v, str) or not v.strip():
                    raise ValueError("ActionRef.confirm values must be non-empty strings")
                if v != v.strip():
                    raise ValueError("ActionRef.confirm values must be normalized")
        if not isinstance(self.policy_sensitive, bool):
            raise ValueError("ActionRef.policy_sensitive must be a bool")
        validate_action_ref(self.as_contract(), normalize_label_payload=False)
    ...
    def from_dict(cls, data: dict[str, Any]) -> ActionRef:
        """Construct an ActionRef from a dictionary representation."""
        if not isinstance(data, dict):
            raise ValueError("ActionRef data must be a dict")
        unexpected_keys = set(data) - {"id", "label", "payload", "confirm", "policy_sensitive"}
        if unexpected_keys:
            field_list = ", ".join(sorted(unexpected_keys))
            raise ValueError(f"Unsupported ActionRef field(s): {field_list}")
        for key in ("id", "label", "payload"):
            if key not in data:
                raise ValueError(f"ActionRef data missing required key: {key}")
        ...
        return cls(
            ...
            confirm=confirm,
            policy_sensitive=data.get("policy_sensitive", False),
        )

    def as_dict(self) -> dict[str, Any]:
        """Return the dictionary representation of this action reference."""
...
class PatchReviewActionSelection:
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> PatchReviewActionSelection:
        """Construct a PatchReviewActionSelection from a dictionary representation."""
        if not isinstance(data, dict):
            raise ValueError("PatchReviewActionSelection data must be a dict")
        ...
        if unexpected_keys:
            field_list = ", ".join(sorted(unexpected_keys))
            raise ValueError(f"Unsupported PatchReviewActionSelection field(s): {field_list}")
        for key in ("kind", "patch_id", "action"):
            if key not in data:
                raise ValueError(f"PatchReviewActionSelection data missing required key: {key}")

        action_data = data["action"]
        if not isinstance(action_data, dict):
            raise ValueError("PatchReviewActionSelection.action must be a dict")
        action = ActionRef.from_dict(action_data)
...
class CompletePatchReviewActions:
    ...
    @classmethod
    def from_dict(cls, data: dict[str, Any]) -> CompletePatchReviewActions:
        """Construct a CompletePatchReviewActions from a dictionary representation."""
        if not isinstance(data, dict):
            raise ValueError("CompletePatchReviewActions data must be a dict")
        unexpected_keys = set(data) - {
            "contract_version",
            "patch_id",
            "flow",
        ...
        if unexpected_keys:
            field_list = ", ".join(sorted(unexpected_keys))
            raise ValueError(f"Unsupported CompletePatchReviewActions field(s): {field_list}")
        for key in ("patch_id", "preview", "decisions"):
            if key not in data:
                raise ValueError(f"CompletePatchReviewActions data missing required key: {key}")

        preview_data = data["preview"]
        if not isinstance(preview_data, dict):
            raise ValueError("CompletePatchReviewActions.preview must be a dict")
        preview = ActionRef.from_dict(preview_data)

        decisions_data = data["decisions"]
        if not isinstance(decisions_data, dict):
            raise ValueError("CompletePatchReviewActions.decisions must be a dict")
        for k in ("apply", "reject"):
            if k not in decisions_data:
                raise ValueError(f"CompletePatchReviewActions.decisions missing required key: {k}")
            if not isinstance(decisions_data[k], dict):
                raise ValueError(f"CompletePatchReviewActions.decisions[{k}] must be a dict")

        apply = ActionRef.from_dict(decisions_data["apply"])
        reject = ActionRef.from_dict(decisions_data["reject"])
    ...
    def as_dict(self) -> dict[str, Any]:
        """Return the dictionary representation of these actions."""
        return self.as_contract()
...
def validate_complete_patch_review_capabilities(capabilities: Any) -> None:
    ...
    missing = [
        ...
        if action_id.endswith("_patch") and not (
            capabilities.supports_action(action_id)
            if hasattr(capabilities, "supports_action")
            else (action_id in supported_actions)
        )
    ]
    if missing:
...
def patch_review_execution_preconditions(control: str) -> dict[str, bool]:
    ...
    normalized_control = _ACTION_TO_CONTROL.get(normalized_control, normalized_control)
...
def validate_patch_review_execution_state(
    *,
    control: str,
    patch_id: str,
    review_state: dict[str, Any] | "PatchReviewState",
) -> None:
    normalized_control = control.strip().lower()
    normalized_control = _ACTION_TO_CONTROL.get(normalized_control, normalized_control)
    if normalized_control not in set(PATCH_REVIEW_REQUIRED_PARTS):
    ...
    if not expected_patch_id:
        raise ValueError("Patch review patch_id is required")
    review_state = _patch_review_state_contract(review_state)
    if not isinstance(review_state, dict):
        raise ValueError("Patch review state must be an object")
    unexpected_keys = set(review_state) - {"patch_id", "previewed", "resolved", "resolved_as"}
    if unexpected_keys:
        field_list = ", ".join(sorted(unexpected_keys))
        raise ValueError(f"Unsupported patch review execution state field(s): {field_list}")
    ...
    resolved = review_state.get("resolved", False)

    if not isinstance(resolved, bool):
        raise ValueError("Patch review state resolved must be a boolean")
    ...
    resolved_as = review_state.get("resolved_as")
    ...
    if resolved:
        if resolved_as not in {"apply", "reject"}:
            raise ValueError(
                "Patch review state resolved_as must be 'apply' or 'reject' when resolved"
            )
        if not previewed:
            raise ValueError("Patch review state cannot be resolved without previewed=True")
    else:
        if resolved_as is not None:
            raise ValueError("Patch review state resolved_as must be None when not resolved")

    if resolved is True:
        resolved_as_val = review_state.get("resolved_as", "unknown")
        raise PermissionError(
            f"Patch review already resolved as {resolved_as_val!r}; cannot execute {normalized_control!r}"
        )
...
def advance_patch_review_state(
    *,
    patch_id: str,
    control: str,
    current_state: dict[str, Any] | PatchReviewState | None = None,
) -> dict[str, Any]:
    """Return the updated review state dict after a control executes successfully.

    For ``preview``: marks the patch as previewed.
    For ``apply`` / ``reject``: requires the current state to already be previewed,
    then marks the patch as resolved.

    Raises ``ValueError`` for unknown controls or an empty ``patch_id``.
    Raises ``PermissionError`` when preconditions are not satisfied.
    """
    ...
    if normalized_control not in set(PATCH_REVIEW_REQUIRED_PARTS):
        raise ValueError(
            f"Unknown patch review control {control!r}; "
            "expected 'preview', 'apply', or 'reject'"
        )
    ...
    if current_state is not None:
        from exegesis_shared.models.patch_review_state import PatchReviewState
        if isinstance(current_state, PatchReviewState):
            state_obj = current_state
        elif isinstance(current_state, dict):
            state_obj = PatchReviewState.from_dict(current_state)
        else:
            raise TypeError("current_state must be a dict, PatchReviewState, or None")
        if state_obj.resolved:
            resolved_as = state_obj.resolved_as or "unknown"
            raise PermissionError(
                f"Patch review already resolved as {resolved_as!r}; "
                f"cannot execute {normalized_control!r}"
            )
        if state_obj.patch_id != normalized_patch_id:
            raise ValueError("Patch review state does not match the current patch")
    preconditions = PATCH_REVIEW_EXECUTION_PRECONDITIONS[normalized_control]
    if preconditions["requires_preview"]:
    ...
    next_state: dict[str, Any] = {"patch_id": normalized_patch_id}
    if normalized_control == "preview":
        next_state["previewed"] = True
    else:
        next_state["previewed"] = True
        next_state["resolved"] = True
        next_state["resolved_as"] = normalized_control
    return next_state


def _patch_review_state_contract(
    review_state: dict[str, Any] | "PatchReviewState",
) -> dict[str, Any]:
    from exegesis_shared.models.patch_review_state import PatchReviewState

    if isinstance(review_state, PatchReviewState):
        return review_state.as_dict()
    if isinstance(review_state, dict):
        try:
            return PatchReviewState.from_dict(review_state).as_dict()
        except ValueError as e:
            msg = str(e)
            if msg.startswith("Unsupported PatchReviewState field(s):"):
                fields = msg.split(":", 1)[1].strip()
                raise ValueError(f"Unsupported patch review execution state field(s): {fields}") from e
            raise
    return review_state


def advance_patch_review_state_typed(
    *,
    patch_id: str,
    control: str,
    current_state: PatchReviewState | None = None,
) -> PatchReviewState:
    """Typed-domain bridge: advance review state using PatchReviewState in/out.
    ...
    ``advance_patch_review_state`` for all precondition and transition logic.

    Raises ``ValueError`` for unknown controls, empty ``patch_id``, or a
    ``current_state`` that is not a ``PatchReviewState``.
    Raises ``PermissionError`` when preconditions are not satisfied.
    """
    from exegesis_shared.models.patch_review_state import PatchReviewState
    ...
    if current_state is not None and not isinstance(current_state, PatchReviewState):
        raise ValueError(
            "advance_patch_review_state_typed: current_state must be a PatchReviewState or None"
        )
    raw_current = current_state.as_dict() if current_state is not None else None
    raw_next = advance_patch_review_state(
        patch_id=patch_id,
        control=control,
        current_state=raw_current,
    )
    return PatchReviewState.from_dict(raw_next)
...
def _validate_patch_review_execution_precondition(
    *,
    control: str,
    patch_id: str,
    review_state: dict[str, Any] | "PatchReviewState" | None,
) -> None:
    normalized_control = control.strip().lower()
    normalized_control = _ACTION_TO_CONTROL.get(normalized_control, normalized_control)
    if normalized_control not in set(PATCH_REVIEW_REQUIRED_PARTS):
...
def canonicalize_action_order(actions: list[Any]) -> list[dict[str, Any]]:
    ...
    for action in actions:
        if not isinstance(action, dict):
            continue
        try:
            validate_action_ref(action, normalize_label_payload=False)
        except ValueError:
            continue
...
def _validate_patch_review_control_action(
    ...
    *,
    ...
    if action.id != expected_action_id:
        raise ValueError(f"Patch review {normalized_control} control must use {expected_action_id}")
    if action.payload.get("patch_id") != expected_patch_id:
        raise ValueError(f"Patch review {normalized_control} control must match the current patch")
    if normalized_control in {"apply", "reject"}:
        ...
        if action.confirm != expected_confirm or action.policy_sensitive is not True:
            raise ValueError(
                f"Patch review {normalized_control} control must be engine-authoritative and policy-gated"
            )
    return action
...
def patch_review_availability_from_contract(review: dict[str, Any]) -> dict[str, Any]:
    ...
    review_ver = review.get("contract_version")
    if review_ver != PATCH_REVIEW_CONTRACT_VERSION or isinstance(review_ver, bool):
    ...
    if (
        ...
        and not isinstance(preview.get("contract_version"), bool)
        ...
        and not isinstance(preview.get("patch_preview_contract_version"), bool)
    ):
    ...
    for entry in decisions:
        ...
        if (
            ...
            and not isinstance(selection.get("contract_version"), bool)
            ...
            and not isinstance(selection.get("patch_decision_contract_version"), bool)
    ...
    *,
    ...
    if capabilities is not None:
        ...
        if not (
            capabilities.supports_action(action_id)
            if hasattr(capabilities, "supports_action")
            else (action_id in set(capabilities.actions_supported))
        ):
...
def patch_review_next_control_from_contract(
    ...
    *,
    ...
    if not isinstance(next_required, str):
        # Contract is complete; return the first canonical control as "available"
        # so callers can still present the preview→apply/reject workflow to the user.
        first_required = next(
            (part for part in PATCH_REVIEW_REQUIRED_PARTS if part in summary["controls"]),
            None,
        )
        if first_required is None:
            return None
        next_required = first_required
        # Recompute aliases for the overridden next_required.
        summary = dict(summary)
        summary["next_required"] = next_required
        summary["next_required_command_aliases"] = list(
            PATCH_REVIEW_CLI_COMMAND_ALIASES.get(next_required, ())
        )
...
def complete_patch_review_available_controls_from_state(
    card: dict[str, Any],
    *,
    patch_id: str,
    review_state: dict[str, Any] | "PatchReviewState" | None = None,
) -> dict[str, Any]:
    expected_patch_id = patch_id.strip()
    if not expected_patch_id:
        raise ValueError("Patch review patch_id is required")
    ...
    controls = patch_review_control_actions_from_contract(card, review, patch_id=expected_patch_id)

    if review_state is None:
        available_controls = ("preview",)
        state_contract = {
            "patch_id": expected_patch_id,
            "previewed": False,
            "resolved": False,
        ...
        state = (
            review_state
            if isinstance(review_state, PatchReviewState)
            else PatchReviewState.from_dict(review_state)
        )
        if state.patch_id != expected_patch_id:
            raise ValueError("Patch review state patch_id does not match the current patch")
        available_controls = state.available_controls()
        state_contract = state.as_dict()

    available = [
        {
            "control": control,
            "action_id": controls[control]["action_id"],
            "command": str(controls[control]["slot"]),
            "command_aliases": list(PATCH_REVIEW_CLI_COMMAND_ALIASES[control]),
            "execution_policy": deepcopy(PATCH_REVIEW_EXECUTION_POLICY[control]),
        }
        for control in PATCH_REVIEW_REQUIRED_PARTS
        if control in set(available_controls)
    ]
    return {
        "contract_version": PATCH_REVIEW_CONTRACT_VERSION,
        "patch_id": expected_patch_id,
        ...
        "available_controls": [entry["control"] for entry in available],
        "available_action_ids": [entry["action_id"] for entry in available],
        "controls": available,
    }
...
def resolve_card_selection_execution(
    ...
    *,
    ...
    if not (
        capabilities.supports_action(action_ref.id)
        if hasattr(capabilities, "supports_action")
        else (action_ref.id in set(capabilities.actions_supported))
    ):
...
def build_patch_preview_selection(card: dict[str, Any], *, patch_id: str) -> dict[str, Any]:
    ...
    preview_ver = patch_preview.get("contract_version")
    if preview_ver != PATCH_PREVIEW_CONTRACT_VERSION or isinstance(preview_ver, bool):
        raise ValueError("Unsupported patch preview contract version")
    ...
    if isinstance(selection, dict):
        sel_ver = selection.get("contract_version")
        if sel_ver != ACTION_SELECTION_CONTRACT_VERSION or isinstance(sel_ver, bool):
            raise ValueError("Unsupported action selection contract version")
        if selection.get("selection_model") != "one_based_action_slot":
            raise ValueError("Unsupported action selection model")
        ...
        if selection.get("action_identity") != entry.get("action_identity"):
            raise ValueError("Patch preview selection does not match the current card")
        _validate_patch_review_selection_metadata(selection)
        sel_preview_ver = selection.get("patch_preview_contract_version")
        if sel_preview_ver != PATCH_PREVIEW_CONTRACT_VERSION or isinstance(sel_preview_ver, bool):
            raise ValueError("Unsupported patch preview contract version")
        if selection.get("patch_id") != expected_patch_id:
            raise ValueError("Patch preview selection does not match the current patch")
...
def build_patch_decision_selection(
    ...
    *,
    ...
    decision_ver = patch_decision.get("contract_version")
    if decision_ver != PATCH_DECISION_CONTRACT_VERSION or isinstance(decision_ver, bool):
        raise ValueError("Unsupported patch decision contract version")
    ...
    if isinstance(selection, dict):
        ...
        if selection.get("slot") != entry.get("slot"):
            raise ValueError("Patch decision selection does not match the current card")
        _validate_patch_review_selection_metadata(selection)
        sel_decision_ver = selection.get("patch_decision_contract_version")
        if sel_decision_ver != PATCH_DECISION_CONTRACT_VERSION or isinstance(sel_decision_ver, bool):
            raise ValueError("Unsupported patch decision contract version")
...
def build_patch_decision_from_engine_loop_outcome(
    card: dict[str, Any],
    outcome: "EngineLoopOutcome",
    *,
    decision: str,
) -> dict[str, Any]:
    """Build a typed patch decision selection from a summarized EngineLoopOutcome.

    Bridges the polling loop (which yields an EngineLoopOutcome dict) to
    ``build_patch_decision_selection`` without requiring the caller to extract
    patch_id manually.  Raises ValueError if the outcome has no patch_id (e.g.
    a retrieval-only run) or if the decision is not valid for the current state.
    """
    from exegesis_shared.models.patch_review_state import PatchReviewState

    state = PatchReviewState.from_engine_loop_outcome(outcome)
    if state is None:
        raise ValueError(
            "outcome contains no patch_id; cannot build a patch decision selection"
        )
    normalized_decision = decision.strip().lower()
    if normalized_decision not in state.available_controls():
        available = ", ".join(repr(c) for c in state.available_controls()) or "<none>"
        raise ValueError(
            f"decision {decision!r} is not available in the current patch review state; "
            f"available controls: {available}"
        )
    return build_patch_decision_selection(card, patch_id=state.patch_id, decision=normalized_decision)


def validate_action_ref(
    action: Any,
    *,
    normalize_check: bool = True,
    normalize_label_payload: bool = True,
) -> None:
    if not isinstance(action, dict):
    ...
    if normalize_check and normalize_label_payload and label != label.strip():
        raise ValueError("Action label must be normalized")
    payload = action.get("payload")
    if not isinstance(payload, dict):
        raise ValueError("Action payload must be an object")
    _validate_action_payload(action_id, payload, normalize_check=(normalize_check and normalize_label_payload))
    confirm = action.get("confirm")
    if confirm is not None:
        if not isinstance(confirm, dict):
            raise ValueError("Action confirm must be an object")
        if "title" not in confirm:
            raise ValueError("Action confirm object must contain 'title'")
        ...
        for key, value in confirm.items():
            ...
            if normalize_check and key != key.strip():
                raise ValueError("Action confirm keys must be normalized")
            if not isinstance(value, str) or not value.strip():
                raise ValueError("Action confirm values must be non-empty strings")
            if normalize_check and value != value.strip():
...
def _action_ref_from_contract(action: dict[str, Any]) -> ActionRef:
    validate_action_ref(action)
    return ActionRef.from_dict(action)
...
def execute_action_with_policy_gate(
    *,
    ...
    if not (
        capabilities.supports_action(action.id)
        if hasattr(capabilities, "supports_action")
        else (action.id in set(capabilities.actions_supported))
    ):
        raise ValueError("Action not supported by client")
...
def engine_authoritative_action_ref(action: ActionRef) -> ActionRef:
    validate_action_ref(action.as_contract(), normalize_label_payload=False)
...
def validate_engine_authoritative_action_ref(action: ActionRef) -> None:
    """Require an action to already match the engine-authoritative contract."""
    validate_action_ref(action.as_contract(), normalize_label_payload=False)
    normalized = engine_authoritative_action_ref(action)
    if normalized.as_contract() != action.as_contract():
        raise ValueError("ActionRef must match engine-authoritative action contract")
...
def execute_retrieval_promote_with_policy_gate(
    *,
    card: dict[str, Any],
    item_id: str,
    capabilities: Any,
    ...
    """Typed entry point for promote_to_basket from a RetrievalResultsCard."""
    if not isinstance(card, dict) or card.get("type") != "RetrievalResultsCard":
        raise ValueError("execute_retrieval_promote_with_policy_gate requires a RetrievalResultsCard")
    if not isinstance(item_id, str) or not item_id.strip():
        raise ValueError("item_id must be a non-empty string")
    if item_id != item_id.strip():
        raise ValueError("item_id must be normalized")
    results = card.get("results", [])
    result_ids = {r.get("item_id") for r in results if isinstance(r, dict)}
    if item_id not in result_ids:
        raise ValueError(f"item_id '{item_id}' not found in RetrievalResultsCard results")
    action = ActionRef(
...
def execute_retrieval_open_corpus_item_with_policy_gate(
    *,
    card: dict[str, Any],
    item_id: str,
    capabilities: Any,
    ...
    """Typed entry point for open_corpus_item from a RetrievalResultsCard."""
    if not isinstance(card, dict) or card.get("type") != "RetrievalResultsCard":
        raise ValueError("execute_retrieval_open_corpus_item_with_policy_gate requires a RetrievalResultsCard")
    if not isinstance(item_id, str) or not item_id.strip():
        raise ValueError("item_id must be a non-empty string")
    if item_id != item_id.strip():
        raise ValueError("item_id must be normalized")
...
def execute_retrieval_pin_to_context_set_with_policy_gate(
    *,
    card: dict[str, Any],
    item_id: str,
    context_set_id: str,
    ...
    """Typed entry point for pin_to_context_set from a RetrievalResultsCard."""
    if not isinstance(card, dict) or card.get("type") != "RetrievalResultsCard":
        raise ValueError("execute_retrieval_pin_to_context_set_with_policy_gate requires a RetrievalResultsCard")
    if not isinstance(item_id, str) or not item_id.strip():
        raise ValueError("item_id must be a non-empty string")
    if item_id != item_id.strip():
        raise ValueError("item_id must be normalized")
    if not isinstance(context_set_id, str) or not context_set_id.strip():
        raise ValueError("context_set_id must be a non-empty string")
    if context_set_id != context_set_id.strip():
        raise ValueError("context_set_id must be normalized")
    normalized_item_id = item_id
    normalized_context_set_id = context_set_id
    results = card.get("results", [])
    result_ids = {r.get("item_id") for r in results if isinstance(r, dict)}
    if normalized_item_id not in result_ids:
        raise ValueError(f"item_id '{normalized_item_id}' not found in RetrievalResultsCard results")
    action = ActionRef(
        id="pin_to_context_set",
        label="Pin to context",
        payload={"item_id": normalized_item_id, "context_set_id": normalized_context_set_id},
        confirm=None,
        policy_sensitive=False,
    )
    return execute_action_with_policy_gate(
...
def execute_basket_gather_context_with_policy_gate(
    *,
    card: dict[str, Any],
    context_set_id: str,
    capabilities: Any,
    ...
    """Typed entry point for gather_context from a BasketCard."""
    if not isinstance(card, dict) or card.get("type") != "BasketCard":
        raise ValueError("execute_basket_gather_context_with_policy_gate requires a BasketCard")
    basket_id = card.get("basket_id")
    if not isinstance(basket_id, str) or not basket_id.strip():
        raise ValueError("BasketCard must have a non-empty basket_id for gather_context")
    if basket_id != basket_id.strip():
        raise ValueError("basket_id must be normalized")
    if not isinstance(context_set_id, str) or not context_set_id.strip():
        raise ValueError("context_set_id must be a non-empty string")
    if context_set_id != context_set_id.strip():
        raise ValueError("context_set_id must be normalized")
    action = ActionRef(
        ...
        payload={"basket_id": basket_id, "context_set_id": context_set_id},
        confirm=None,
        policy_sensitive=False,
    )
    return execute_action_with_policy_gate(
...
def execute_basket_create_context_set_with_policy_gate(
    *,
    card: dict[str, Any],
    name: str,
    capabilities: Any,
    ...
    """Typed entry point for create_context_set from a BasketCard."""
    if not isinstance(card, dict) or card.get("type") != "BasketCard":
        raise ValueError("execute_basket_create_context_set_with_policy_gate requires a BasketCard")
...
def execute_context_set_pin_with_policy_gate(
    *,
    card: dict[str, Any],
    item_id: str,
    capabilities: Any,
    ...
    """Typed entry point for pin_to_context_set from a ContextSetCard."""
    if not isinstance(card, dict) or card.get("type") != "ContextSetCard":
        raise ValueError("execute_context_set_pin_with_policy_gate requires a ContextSetCard")
    context_set_id = card.get("context_set_id")
    if not isinstance(context_set_id, str) or not context_set_id.strip():
        raise ValueError("ContextSetCard must have a non-empty context_set_id for pin_to_context_set")
    if context_set_id != context_set_id.strip():
        raise ValueError("context_set_id must be normalized")
    if not isinstance(item_id, str) or not item_id.strip():
        raise ValueError("item_id must be a non-empty string")
    ...
    item_ids = {i.get("item_id") for i in items if isinstance(i, dict)}
    if normalized_item_id not in item_ids:
        raise ValueError(f"item_id '{normalized_item_id}' not found in ContextSetCard items")
    action = ActionRef(
        id="pin_to_context_set",
        label="Pin to context",
        payload={"item_id": normalized_item_id, "context_set_id": normalized_context_set_id},
...
def execute_context_set_open_corpus_item_with_policy_gate(
    *,
    card: dict[str, Any],
    item_id: str,
    capabilities: Any,
    ...
    """Typed entry point for open_corpus_item from a ContextSetCard."""
    if not isinstance(card, dict) or card.get("type") != "ContextSetCard":
        raise ValueError("execute_context_set_open_corpus_item_with_policy_gate requires a ContextSetCard")
    if not isinstance(item_id, str) or not item_id.strip():
        raise ValueError("item_id must be a non-empty string")
    if item_id != item_id.strip():
        raise ValueError("item_id must be normalized")
    ...
    if item_id not in item_ids:
        raise ValueError(f"item_id '{item_id}' not found in ContextSetCard items")
    action = ActionRef(
        id="open_corpus_item",
        label="Open",
        payload={"item_id": item_id},
...
def execute_context_set_promote_to_basket_with_policy_gate(
    *,
    card: dict[str, Any],
    item_id: str,
    capabilities: Any,
    ...
    """Typed entry point for promote_to_basket from a ContextSetCard."""
    if not isinstance(card, dict) or card.get("type") != "ContextSetCard":
        raise ValueError("execute_context_set_promote_to_basket_with_policy_gate requires a ContextSetCard")
    if not isinstance(item_id, str) or not item_id.strip():
        raise ValueError("item_id must be a non-empty string")
    if item_id != item_id.strip():
        raise ValueError("item_id must be normalized")
...
def _validate_action_payload(action_id: str, payload: dict[str, Any], *, normalize_check: bool = True) -> None:
    ...
    for key in payload:
        ...
        if key != key.strip():
            raise ValueError(f"Payload field names must be normalized for action '{action_id}'")
    ...
    for key, value_type in schema.items():
        ...
        if value_type is str:
            ...
            if normalize_check and payload[key] != payload[key].strip():
                raise ValueError(f"Payload field '{key}' must be normalized for action '{action_id}'")
