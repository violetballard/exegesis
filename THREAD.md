...
- Review basis: current branch tip after the `20260429T011323Z` reviewer-fix commit.
...
- Gate-policy edits: no net change after this fixer pass; `scripts/scope-check.sh` is restored to the implementation anchor.
...
4. Ownership/accounting lists the approved shared test edit and restores the prior `scripts/scope-check.sh` policy edit out of the net diff.
