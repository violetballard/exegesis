...
- Causal link: if the `open project/document` command surface drifts away from the canonical catalog without a hard failure, the operator cannot reliably enter the CLI-first MVP loop at all; failing fast on parser/catalog drift hardens that exact first step rather than adding a new one.
...
- Final verification note: this metadata-only fixer pass on `2026-04-24` revalidated the corrected handoff mapping and reran the full required gate set from the current branch tip without changing the reviewed implementation slice.
