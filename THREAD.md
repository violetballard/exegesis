...
- Corrected review target: the actual branch tip after this required-fix commit, superseding rejected target `0fb860a1c160321585d711911bfce0c2f2242d07`.
...
- Demo-path impact: this branch makes `retrieve relevant material` and `gather context into basket` more real by preventing parser/catalog drift for retrieval and `context-basket` CLI fallback commands; it also makes `open project/document`, `apply/reject patch`, and `persist state` more concrete through catalog-owned MVP smoke argv.
...
- Required gates for the corrected final tree: `make scope-check`, `./quality-format.sh --check`, `./quality-lint.sh`, `./quality-test.sh`, `./typecheck-test.sh`, and `make ci` all pass.
