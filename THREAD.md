...
- Implementation slice: `src/qual/commands/catalog.py` and shared-by-approval test exception `tests/unit/test_commands_catalog.py`; no integrator-locked files were changed in the reviewed implementation commit. This fixer tightens the canonical CLI-token contract, adds the `diff-preview` alias-replacement and alias-before-canonical regressions, and refreshes packet metadata.
