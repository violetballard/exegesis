...
- Final revalidation note: this metadata-only fixer refresh reran `make scope-check`, `./quality-format.sh --check`, `./quality-lint.sh`, `./quality-test.sh`, `./typecheck-test.sh`, and `make ci` at the post-review-fix branch tip without changing the reviewed implementation slice.
