...
- Scope / plan alignment: this handoff directly advances the canonical demo-path step `preview and apply or reject a patch` by proving the current CLI fallback rejects parser/catalog drift from `src/main.py::_dispatch()` before the operator reaches the review/apply entrypoints.
