...
- Concrete canonical mapping: this slice makes the exact canonical step `preview and apply or reject a patch` more real inside `project-open/bootstrap -> retrieval -> plan-or-revise -> apply-or-reject -> export-handoff`, so the current engine-first loop still reaches that review/apply step through deterministic `preview`, `apply`, and `reject` entrypoints while Textual remains disabled.
...
- Concrete canonical-path blocker removed: the direct blocker on `preview and apply or reject a patch` is removed because the review/apply CLI entrypoints can no longer silently drift at the alias or token level while still looking stable by canonical command name.
- Roadmap alignment note: this is Milestone 3 scope because it hardens the current manual CLI-first loop at `preview and apply or reject a patch`, keeping that review/apply engine contract deterministic and migration-safe for the upcoming console client while Textual remains disabled; it does not claim broader CLI polish, new workflow reachability, auditable-state progress, or extra engine behavior.
